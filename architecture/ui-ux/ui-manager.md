...
- `#include "src/interfaces/IHUDBadgeNotifier.h"` — full include required (not a forward declaration) because `UIManager` inherits from `IHUDBadgeNotifier`. A forward declaration is insufficient when a class inherits from an interface.

```cpp
class UIManager : public IHUDBadgeNotifier {
public:
    ...
    // IHUDBadgeNotifier -- delegates to m_hud->incrementNotificationBadge(); null-guarded.
    void incrementNotificationBadge() override;
