...
- **(a) Developer-side manual gate only**: This is a **developer-side manual verification step**, NOT a CI pipeline step. There is no automated CI job that runs this command — the developer must run it manually from a terminal before pushing the Phase 1 atomicity commit. Windows CI and Linux CI do not execute this check.
...
- **(d) Windows CI does not run this command**: The `build-windows` and `coverage-windows` CI jobs do not include a GLEW port verification step. This gate applies to the developer's local environment only and is verified by the developer as part of the Phase 1 pre-merge checklist.
