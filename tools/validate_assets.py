...
def _parse_ply_positions(path):
    """Read PLY vertex positions from a binary_little_endian PLY file.

    Returns Irrlicht (X, Y, Z) tuples after applying CPLYMeshFileLoader's axis
    remapping: PLY x->Irrlicht X, PLY y->Irrlicht Z, PLY z->Irrlicht Y (up).
    This matches the B3D coordinate space (Y=up, Z=depth) so callers can use
    the same logic for both file formats.
    ...
    try:
        with open(path, 'rb') as f:
            ...
            for _ in range(vertex_count):
                ...
                if len(vertex_data) < stride:
                    break
                ply_x = struct.unpack_from('<f', vertex_data, offsets[x_idx])[0]
                ply_y = struct.unpack_from('<f', vertex_data, offsets[y_idx])[0]
                ply_z = struct.unpack_from('<f', vertex_data, offsets[z_idx])[0]
                # Irrlicht CPLYMeshFileLoader maps: PLY x->Pos.X, PLY y->Pos.Z, PLY z->Pos.Y.
                # Return Irrlicht (X, Y, Z) so callers work in the same coordinate space
                # as B3D-sourced positions (where Y=up, Z=depth).
                positions.append((ply_x, ply_z, ply_y))
            return positions
...
def check_4b_building_bbox(assets_dir):
    ...
    errors = []
    checked = 0
    # Tight tolerance for B3D (programmatic ground quads hit the exact extent).
    # PLY models from Tripo3D are organic meshes whose footprint may fall up to
    # 1 m short of the tile boundary — use a larger tolerance for those files.
    B3D_TOL = 1e-4
    PLY_TOL = 1.0
    ...
    for asset_name in _PHASE_11E_CELL_ASSIGNMENT:
        ...
        # PLY buildings are organic Tripo3D meshes — they don't have programmatic
        # ground quads that span the exact tile footprint, so skip the XZ extent
        # check for PLY files.  B3D buildings (generated by generate_b3d_models.py)
        # do have explicit ground quads and must still satisfy the extent constraint.
        fh = _expected_fh(asset_name)
        if fh is not None and not is_ply:
            if max_x < fh - TOL:
                asset_errors.append(
