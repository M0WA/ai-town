...
| Megalopolis | 500,000+ |
  ...
  **CityRatingTier C++ enum**: The `CityRatingTier` enum must be defined in `src/interfaces/simulation_types.h` and used as the return type of `ICitySimulation::getCityRating() const`. The C++ enum definition is:

  ```cpp
  enum class CityRatingTier { Village, Town, City, Metropolis, Megalopolis };
  ```

  `MockCitySimulation` must include `MOCK_METHOD(CityRatingTier, getCityRating, (), (const, override))`.
