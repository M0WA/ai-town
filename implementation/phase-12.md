## Phase 12: Polish, Performance & V1 Hardening
...
- [ ] **Performance**: 60 FPS on mid-range desktop GPU with 10,000 vehicle agents; ≤2,000 draw calls per frame; simultaneous scene texture VRAM ≤180 MB (per `architecture/asset-standards/2d-texture-standards.md` §Scene VRAM Budget — updated for Phase 11e 4096 building atlas; actual worst-case sum ≈166.5 MB); total resident texture VRAM ≤1.0 GB ceiling (GPU headroom on 4 GB card). Both constraints must hold simultaneously. (ref: `architecture/game-design/minimum-viable-simulation.md`, `architecture/asset-standards/2d-texture-standards.md`)
...
- [ ] **Coverage gate**: Verify the `coverage-linux` lcov gate (established at Phase 5 and enforced at ≥80% from Phase 5 onward) still passes after all Phase 12 changes. This is NOT a new gate — it is a verification that Phase 12 optimizations, refactors, and QA fixes have not regressed coverage below 80% on `src/simulation/`, `src/terrain/`, `src/ui/`; `src/rendering/`, `src/audio/`, `src/platform/` remain excluded. (ref: `architecture/testing/coverage.md`)
- [ ] **Colorblind mode QA verification pass** — no new functional implementation in Phase 12; Phase 8 delivers all colorblind mode code. This deliverable is a joint `gamedesign-ux` + `graphics-dev-irrlicht` QA review confirming the Phase 8 implementation is correct and complete: pattern-supplemented minimap palette renders correctly at both 1280×720 and 1920×1080; R/C/I demand bar labels always present (color supplemental, not color-only); service overlay geometric patterns visible and distinguishable; zone cursor zone-type label overlay present. Any implementation gaps discovered during QA are fixed before Phase 12 exit. (ref: `architecture/ui-ux/resolution-ui-scaling.md`)
...
- **RISK**: 10,000-agent simulation performance may require traffic pathfinding optimization. **Spike**: profile A* at 10,000 agents on a 1024×1024 map with standard grid road spacing early in Phase 12.
- **RISK**: `coverage-linux` gate may fall below 80% if `src/ui/` UIManager methods are incompletely tested via `MockUIBackend`. **Spike**: run lcov locally after Phase 8 to check `src/ui/` line coverage before declaring Phase 8 done.
