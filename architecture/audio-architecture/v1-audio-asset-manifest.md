...
| check_21 | `sfx_zone_*.ogg` | Phase 10 — reserved. Zone loop silence-floor amplitude gate: for each `sfx_zone_*.ogg`, decode the OGG and verify the leading `ceil(44100 × 0.1) = 4410` samples and trailing 4410 samples are all at or below −60 dBFS peak amplitude. Graceful no-op if no files exist. | Any zone loop file where leading or trailing 4410 samples exceed −60 dBFS peak amplitude |
