...
- [x] `AudioSystem` stub header `src/audio/audio_system.h` (`sound-dev-opensoftal`): header-only stub (no AL code, no implementation). Declares:
...
- [x] `src/interfaces/audio_types.h` — **Phase 0 created this file as a stub. Phase 3 EXTENDS and VERIFIES it — not re-authors it from scratch.** Phase 3 verifies or adds: `using SoundId = uint32_t`; `using MusicTrackId = uint32_t`; `using SoundHandle = uint32_t`; `enum class TimeOfDay { DAY, DUSK, NIGHT, DAWN }`; `enum class SoundPriority { LOW = 0, NORMAL = 1, HIGH = 2, CRITICAL = 3 }`. Phase 3 adds the `StingerType` enum class and source pool layout constants in the canonical declaration order (constants first, enum second, static_asserts third):
  ...
  Phase 3 also adds `constexpr float kZoneLoopMaxPreloadDurationSeconds = 18.0f;` and `constexpr float kVehicleEngineLoopMinDurationSeconds = 6.0f;` — both declared alongside the pool constants. Phase 5 `validate_assets.py` MUST reference `kVehicleEngineLoopMinDurationSeconds` rather than hardcoding the literal 6.0 (per `architecture/audio-architecture/v1-audio-asset-manifest.md` lines 149–157). Do NOT define `SimSpeed` or `SpeedMultiplier` in `audio_types.h`. (ref: `architecture/audio-architecture/audio-system.md`, `architecture/audio-architecture/source-pool.md`)

  > **Migration note**: The pool constants (`kEvictableSFXCount`, `kStingerCount`, `kSFXPoolSize`, `kStreamSourceCount`, `kTotalSources`, `kTransientReserveStart`) currently also exist in `src/audio/audio_constants.h`. As part of this deliverable, `audio_constants.h` must be updated to `#include "src/interfaces/audio_types.h"` and its own declarations of those constants removed to avoid redefinition errors. `audio_smoke_test.cpp` continues to include `audio_constants.h` and will get the constants transitively.

- [x] **`src/audio/al_check.h` Phase 3 stub** (`sound-dev-opensoftal`): create `src/audio/al_check.h` with two inline no-op stub functions. Must NOT include any OpenAL headers:
  ...
  **`alcCheckError` call site contract (locked Phase 3)**: Phase 7 call sites pass `ALCdevice*` directly to `alcCheckError` — the implicit `ALCdevice*` to `void*` conversion is defined behavior in C++ and no explicit cast is required. Example: `alcCheckError(m_device, "alcMakeContextCurrent");` (ref: `architecture/audio-architecture/error-checking.md § Call sites`)
- [x] **libvorbisfile and RapidCheck linkage in `audio_tests`**: add `Vorbis::vorbisfile`, `rapidcheck`, and `rapidcheck_gtest` to `audio_tests` `target_link_libraries` now. **`audio_tests` include directories** — the Phase 3 CMake amendment must verify that `target_include_directories(audio_tests PRIVATE ...)` includes ALL of: `tests/simulation/`, `src/interfaces/`, `src/audio/`, and `${CMAKE_SOURCE_DIR}`. (ref: `architecture/ci-cd/dependency-management.md`, `architecture/testing/testability-architecture.md`)
- [x] `MockAudioSystem` in `tests/simulation/mock_audio_system.h` with GMock `MOCK_METHOD` declarations for all 11 `IAudioSystem` methods. **Header isolation constraint**: `mock_audio_system.h` MUST NOT include any `src/ui/` or `tests/ui/` headers — only `IAudioSystem.h`, `audio_types.h`, and GMock headers. Using opaque handle types (`SoundHandle`, `MusicTrackId`) means no UI types need to be referenced. Violating this creates circular include paths when `ui_tests` includes both `mock_audio_system.h` and UI headers. **Compile-smoke requirement**: `MockAudioSystem_InstantiatesCleanly` test in `tests/audio/audio_smoke_test.cpp` passes — confirms all 11 vtable methods are declared and `NiceMock<MockAudioSystem>` is fully constructible. (ref: `architecture/audio-architecture/audio-system.md`, `architecture/testing/testability-architecture.md`)

#### Simulation Interface & Types

- [x] `ICitySimulation.h` in `src/interfaces/` with minimum method signatures: `setPaused(bool)`, `isPaused() const`, `setSpeed(SpeedMultiplier)`, `getSpeedMultiplier() const`, `hasUndoPendingAction() const`, `getUndoExpiryTimeSeconds() const`, `CityRatingTier getCityRating() const`, `getDemandPressurePct(ZoneType) const`, `getTreasuryBalance() const`, `getCurrentMonthlyRevenue() const`, `getOutstandingDebt() const`, `estimateMonthlyUpkeep() const`, `getNextUnlockThreshold(Difficulty) const`, `getTotalPopulation() const`, `getConsecutiveDeficitMonths() const`, `getTrafficDemandFactor(ZoneType) const`, `getDensityUnlockState() const`. **`ICitySimulation` extends `ISimulationPauser`**: `class ICitySimulation : public ISimulationPauser`. `ICitySimulation.h` must `#include "simulation_types.h"` to get `Difficulty`, `ZoneType`, `SpeedMultiplier`, and `DensityUnlockState` as complete types. `MockCitySimulation` in `tests/ui/mock_city_simulation.h` with corresponding `MOCK_METHOD` declarations. (ref: `architecture/testing/testability-architecture.md`)

  > **Breaking change**: `getCityRating()` is committed in the codebase as returning `int`. Phase 3 changes this to return `CityRatingTier`. The `ICitySimulation.h` interface, `MockCitySimulation.h` mock, and any test fixtures using a default return value of `getCityRating()` must all be updated atomically in the same commit.

  **Default starting speed**: `kDefaultSimSpeed = SpeedMultiplier::x3` MUST be defined in `simulation_types.h`. In `MockCitySimulation` fixtures, use `ON_CALL(mock, getSpeedMultiplier()).WillByDefault(Return(SpeedMultiplier::x3))` as the correct default.

- [x] **`CityRatingTier` enum** — add to `simulation_types.h` as a Phase 3 deliverable:

  ```cpp
  enum class CityRatingTier { Village, Town, City, Metropolis, Megalopolis };
  ```

- [x] `ManualRNG` in `tests/simulation/manual_rng.h`: two independent sequences (`intSeq` and `floatSeq`); strict mode (default) throws `std::logic_error` on sequence exhaustion; validates all `floatSeq` values in [0.0, 1.0) at construction time with a throw; non-strict mode wraps around. **`ManualRNG::verifyAllConsumed()` method**: asserts `m_intIdx == m_intSeq.size()` AND `m_floatIdx == m_floatSeq.size()`. Call `rng_.verifyAllConsumed()` in `TearDown()` for any fixture using `ManualRNG` in strict mode. **`ManualRNG` self-tests** (`test-dev-cpp`): add all 6 required named test cases in `tests/simulation/manual_rng_test.cpp`:
...
- [x] `ManualClock` in `tests/simulation/manual_clock.h` (ref: `architecture/testing/testability-architecture.md`). **`ManualClock` must also be included in the `tests/audio/` CMake target's include directories** — add `tests/simulation/` to `target_include_directories(audio_tests PRIVATE ...)`.
- [x] `MockRenderer` in `tests/simulation/mock_renderer.h` returning incrementing non-zero `TextureHandle` values starting from 1.
...
- [x] `WallClock` implementation in `src/platform/WallClock.cpp` — Phase 3 VERIFIES that `src/platform/WallClock.cpp` is present and contains a `nowSeconds()` body using `std::chrono::steady_clock` (Phase 1 deliverable — do NOT re-author). If the file is absent or the body is missing, that is a Phase 1 regression that must be fixed before Phase 3 closes. (ref: `architecture/testing/testability-architecture.md`)
- [x] `NullSimulationPauser` in `src/interfaces/null_simulation_pauser.h` — no-op `ISimulationPauser` implementation. (ref: `architecture/testing/testability-architecture.md`)

#### UI Interface & UIManager Shell

- [x] `IUIBackend` interface in `src/ui/`: all required methods — `addStaticText`, `addButton`, `removeElement`, `setElementText`, `setElementVisible`, `isElementVisible`, `setElementEnabled`, `isElementEnabled`, `setElementAlpha`, `setElementImage`, `getElementText`, `getElementRect`, `getScreenWidth`, `getScreenHeight`, `getVirtualWidth() const`, `getVirtualHeight() const`, `UIElementHandle loadTexture(const std::string& path)` — **17 methods total (including `loadTexture()`)**. **`loadTexture()` return type**: returns `UIElementHandle` — callers pass the returned handle to `setElementImage()` to display loaded textures on UI elements. Returns `kInvalidUIElement` on failure. **`IUIBackend.h` MUST be placed in `src/ui/` (NOT `src/interfaces/`).** `IrrlichtUIBackend` stub in `src/rendering/` must provide no-op stub implementations for ALL 17 methods. **Testability isolation constraint**: `IrrlichtUIBackend.h` MUST use forward declarations for all Irrlicht types (`class irr::IrrlichtDevice;`, `class irr::gui::IGUIEnvironment;`, etc.) in the header — Irrlicht headers are included ONLY in `IrrlichtUIBackend.cpp`. This prevents Irrlicht headers from contaminating `src/ui/` headers, which must remain testable in isolation without an Irrlicht device (per `architecture/graphics-architecture/irrlicht-device-lifecycle.md`). (ref: `architecture/testing/testability-architecture.md`, `architecture/ui-ux/ui-manager.md`, `architecture/graphics-architecture/irrlicht-device-lifecycle.md`)
- [x] `UIElementHandle` (uint32_t), `kInvalidUIElement = 0`, `Rect` struct defined before `IUIBackend` in `IUIBackend.h` (ref: `architecture/testing/testability-architecture.md`)
- [x] `MockUIBackend` in `tests/ui/mock_ui_backend.h` (ref: `architecture/testing/testability-architecture.md`)
- [x] `MockSimulationPauser` in `tests/ui/mock_simulation_pauser.h` with `MOCK_METHOD(void, setPaused, (bool), (override))`. **Phase 6 `NotificationManager` auto-pause tests use `MockCitySimulation` (NOT `MockSimulationPauser`)** — `MockSimulationPauser` is used only for contexts where a bare `ISimulationPauser*` is needed. (ref: `architecture/testing/testability-architecture.md`)
- [x] **Phase 3 VERIFIES** that `UIScaler::unproject()` and `UIScaler::getViewportRect()` are implemented (Phase 1 deliverables — do NOT re-author). If either is absent or still a stub returning zero/empty, that is a Phase 1 regression requiring a fix before Phase 3 can close. Phase 3 confirms `UIScaler` has constructor signature `UIScaler(int virtualW, int virtualH, int viewportW, int viewportH, int offsetX, int offsetY)`. **`UIScaler` must expose `VirtualPoint unproject(int physicalX, int physicalY) const;` as a public method, where `VirtualPoint` is a nested struct `{ int x; int y; }` declared INSIDE `UIScaler`.** **`UIScaler` unit tests — VERIFY**: all 5 named `UIScaler` unit tests from Phase 1 are Phase 1 deliverables. Phase 3 verifies that they still pass after Phase 3 UIScaler VERIFY. Phase 3 adds one additional compile-only stub:
...
- [x] **`NotificationManager` stub class** (`gamedesign-ux`): `src/ui/notification_manager.h` / `notification_manager.cpp`: constructor signature `NotificationManager(IUIBackend* backend, ICitySimulation* sim, IClock* clock)`. **Reason for `ICitySimulation*` (not `ISimulationPauser*`)**: `NotificationManager` calls `m_sim->setPaused(true)` when the CRITICAL toast queue transitions from empty to non-empty (auto-pause per notification-system.md). `setPaused()` is defined on `ISimulationPauser` (base of `ICitySimulation`). `ICitySimulation*` is passed directly — `UIManager` already holds `m_sim` as `ICitySimulation*`, so no downcast is needed. `NotificationManager` does NOT call `getConsecutiveDeficitMonths()` — `UIManager::update()` is the exclusive polling bridge for deficit-month-based toast dispatch (GD-H3).

  NOTE: If a Phase 1/2 stub created `NotificationManager` with constructor `(IUIBackend*, IClock*, ISimulationPauser*)`, Phase 3 MUST correct it to `(IUIBackend*, ICitySimulation*, IClock*)` before Phase 3 exits.

  Phase 6 signatures locked in Phase 3 even though bodies are Phase 6 deliverables:
  ...
  - `void setModalActive(bool active)` — no-op in Phase 3; Phase 6 sets `m_modalActive = active`; when `active == true`, NotificationManager suppresses its own `setPaused(true)` call (no direct UIManager reference needed — UIManager pushes state via this method).
...
- [x] `UIManager` shell: owns `IGUIEnvironment*`; `GameState` enum (`MainMenu`, `Gameplay`, `Paused`, `GameOver` — V1 canonical; `PostWinFreePlaying` is post-V1 and MUST NOT be added); construction order per `architecture/ui-ux/ui-manager.md`: `NotificationManager` (FIRST — invariant) → `MainMenuPanel` (stub; calls `m_mainMenu->show()` in constructor) → `HUD` → `TaxRatePanel` → `Minimap` → `InspectorPanel` → `PauseMenuPanel` (stub, accepts only `IUIBackend*`, exposes `void setSettingsPanel(SettingsPanel*)` setter) → `SettingsPanel` (stub, empty constructor accepting `IUIBackend*`) → `ModalDialog`; draw order (exactly 10 slots): (1) MainMenuPanel → (2) Minimap → (3) HUD → (4) TaxRatePanel → (5) InspectorPanel → (6) NotificationManager toast stack → (7) PauseMenuPanel → (8) SettingsPanel → (9) Background scrim (modal-active) → (10) ModalDialog (topmost). **All 10 draw-order slots in `UIManager::draw()` MUST be wired in Phase 3.** **Event dispatch chain** — all 6 priority tiers structurally present in Phase 3 (stubs acceptable; branching NOT optional). Priority 2 dual-guard (LOCKED): skipped when `m_notifications->hasCriticalToastVisible()` returns false OR `hasActiveModal()` returns true — i.e., the guard fires (blocks lower-priority input) only when a CRITICAL toast is visible AND no modal is active (consistent with `architecture/ui-ux/input-arbitration.md` lines 9–14: `if (criticalVisible && !modalActive)`). **Toolbar carve-out pixel bounds** in `src/ui/ui_constants.h`: `kToolbarLeft=8`, `kToolbarRight=72`, `kToolbarTop=64`, `kToolbarBottom=784` (all in 1920×1080 virtual space). (ref: `architecture/ui-ux/ui-manager.md`, `architecture/ui-ux/input-arbitration.md`)
  ...
  **Phase 3 UIManager required public method stubs** (all locked now to prevent Phase 6 header breaks):
  ...
  - `bool hasActiveModal() const` — stub returns `false` in Phase 3; Phase 6 implements actual tracking (`m_modalDialog->isActive()`). Used by UIManager internally in the Priority 2 guard (`UIManager::onEvent()`) and in `update()` to coordinate modal state. UIManager PUSHES modal state to NotificationManager via `m_notifications->setModalActive(true/false)` — NOT a back-reference. See Fix 3.
...
- [x] **Minimap, TaxRatePanel, and InspectorPanel stub classes** (`gamedesign-ux`): all three in `src/ui/`, each with constructor accepting `IUIBackend*`; public methods `show()` (no-op), `hide()` (no-op), `draw()` (calls `m_backend->setElementVisible(kSentinel, true)`), **`Rect getBounds() const`** (stub returning `{0,0,0,0}`). Using `irr::core::rect<s32>` as the return type is prohibited — pulls Irrlicht headers into `src/ui/` headers, violating testability isolation. (ref: `architecture/testing/testability-architecture.md`)
...
- [x] **`src/interfaces/LoanTerms.h` stub** (`test-dev-cpp`): `struct LoanTerms { float amount{0.0f}; int repaymentTicks{0}; float interestRate{0.05f}; };`
- [x] **`src/ui/key_bindings.h` stub** (`graphics-dev-irrlicht`): contains default values for all hotkeys from `architecture/ui-ux/hotkey-scheme.md`, a `load(const std::string& path)` method stub, and an `isReservedKey(const std::string& key)` method stub that returns `true` for "Q" and "E". `CameraController` must accept a `const KeyBindings&` reference. Three test cases in `tests/ui/key_bindings_test.cpp`: `KeyBindings_IsReservedKey_Q_ReturnsTrue`, `KeyBindings_IsReservedKey_E_ReturnsTrue`, `KeyBindings_IsReservedKey_W_ReturnsFalse`. (ref: `architecture/ui-ux/hotkey-scheme.md`, `architecture/testing/testability-architecture.md`)
- [x] **HUD class stub** in `src/ui/hud.h` (`gamedesign-ux`): constructor accepts **4 parameters**: `IUIBackend* backend`, `IAudioSystem* audio`, `ICitySimulation* sim`, `IClock* clock`. ALL FOUR parameters MUST be stored as member variables. **Constructor parameter order matches UIManager**: both UIManager and HUD use the canonical order `(backend, audio, sim, clock)` — `ICitySimulation*` before `IClock*`. **HUD owns `BudgetDetailPanel` internally** — `BudgetDetailPanel` is NOT a top-level UIManager panel. Phase 3 HUD stub: (a) forward-declare `class BudgetDetailPanel;` in `hud.h`; (b) declare `BudgetDetailPanel* m_budgetDetail{nullptr};`; (c) create companion stub `src/ui/budget_detail_panel.h`; (d) declare `UIElementHandle m_unsavedDotHandle{kInvalidUIElement}`.

  **`m_unsavedDotHandle` ownership note**: HUD owns the resource bar and all its sub-elements, including the unsaved-changes dot (shown when `setUnsavedChanges(true)` is called on `UIManager`). Phase 8 wires this handle to a dot element in the resource bar area (ref: `architecture/ui-ux/hud-layout.md`).

  (ref: `architecture/ui-ux/hud-layout.md`, `architecture/ui-ux/ui-manager.md`, `architecture/testing/testability-architecture.md`)

#### Camera Controller Tests

- [x] Camera controller unit tests in `tests/ui/camera_controller_test.cpp`. The following 9 named test cases (all Phase 1 deliverables) are REQUIRED to pass before Phase 4 begins:
...
- [x] **`UIManagerDrawOrderTest` fixture** (`test-dev-cpp`): a fixture using **`NiceMock<MockUIBackend>`** (NOT `StrictMock`) with GMock `InSequence` to verify all 10 draw slots are called in the correct Z-order sequence when `UIManager::draw()` is invoked. **Mock policy: NiceMock for ALL mock panels and the backend mock.** Panel sentinel constants live in test-only header `tests/ui/panel_sentinel_handles.h` — NOT in any production panel header. (ref: `architecture/testing/testability-architecture.md`, `architecture/ui-ux/ui-manager.md`)
  ...
  **`ModalDialog` Phase 3 stub `m_active` requirement**: `ModalDialog` Phase 3 stub MUST store `bool m_active{false}`; `show()` sets `m_active = true`; `isActive()` returns `m_active`. **`ModalDialog` Phase 3 stub constructor signature**: `explicit ModalDialog(IUIBackend* backend, ICitySimulation* sim)`. Both parameters MUST be stored as private member variables (`m_backend` and `m_sim`) for use in Phase 6 auto-pause implementation (`m_sim->setPaused(true)` when modal opens and pauses the simulation, tracked via `m_didPauseSim`). Phase 3 stub body is empty; Phase 6 implements the auto-pause logic. (ref: `architecture/testing/testability-architecture.md`)

- [x] **`UIManagerModalTest` fixture TearDown contract** (`test-dev-cpp`): the `UIManagerModalTest` fixture must define an explicit `TearDown()` that calls `ui_.reset()`. **Required member declaration order**: `NiceMock<MockUIBackend> backend_` (first), `NiceMock<MockAudioSystem> audio_` (second), `NiceMock<MockCitySimulation> sim_` (third), `std::unique_ptr<UIManager> ui_` (last). **Destruction order invariant**: `std::unique_ptr<UIManager> ui_` is declared last and therefore destroys first (C++ reverse-destruction order). Mock members (`backend_`, `audio_`, `sim_`) are declared before `ui_` and outlive it. This ordering is MANDATORY — `UIManager`'s destructor may call backend methods during teardown; if `backend_` is destroyed before `ui_`, those calls are use-after-free. Do NOT reorder member declarations. TearDown calls `ui_.reset()` explicitly to trigger UIManager destruction while all mocks are still alive, so EXPECT_CALL verifications fire before mock destructors. **Mock policy for `UIManagerModalTest`**: ALL THREE mock members must use `NiceMock`.
...
- [x] **Integration test routing verification (VERIFY Phase 1 deliverable)** (`cicd-dev-github`): The 'Verify integration test routing (non-zero discovery)' CI step is a Phase 1 deliverable per `architecture/ci-cd/github-actions-workflow.md` line 76. Phase 3 VERIFIES that the step is present in both `build-linux` and `coverage-linux` jobs in `.github/workflows/ci.yml`. The step must run `ctest -N -L '^integration$'` and fail if 0 tests are discovered. This step must already be present from Phase 1 (co-landed with the initial `integration_tests` CMake target registration and the `irrlicht_ui_backend_compile_test.cpp` compile-check test). Phase 3 does NOT re-add this step — Phase 3 VERIFIES it remains in the workflow. If the step is missing from either job, that is a Phase 1 regression requiring remediation before Phase 3 closes.

#### CMake Test Targets

- [x] **`ui_tests` CMake target — consolidated final form (Test-C1)**:
...
- [x] **`terrain_tests` CMake target skeleton** (`test-dev-cpp`): create `terrain_tests` CMake target with stub source file `tests/terrain/terrain_stub.cpp` (single `TEST` calling `SUCCEED()`). Apply `aitown_add_tests(terrain_tests LABEL "unit" TIMEOUT 300 DISCOVERY_TIMEOUT 60)`. Link `aitown_terrain GTest::gtest_main GTest::gmock rapidcheck rapidcheck_gtest` proactively.
...
- [x] **`integration_tests` CMake target**: initial source file `tests/integration/irrlicht_ui_backend_test.cpp` contains a single `EDT_NULL` test that constructs `IrrlichtUIBackend` with an `EDT_NULL` device and calls `addStaticText()`, asserting a non-zero handle is returned; registered via `aitown_add_tests(integration_tests LABEL "integration")`.
  ...
  **The existing Phase 0 `integration_tests` `target_link_libraries` must be amended in Phase 3 to add `aitown_render aitown_ui`**. **Atomicity requirement**: this CMake amendment MUST be committed in the same commit as the `integration_tests` source files (e.g., `irrlicht_ui_backend_test.cpp`). Amending `target_link_libraries` in a separate commit before `aitown_render` and `aitown_ui` CMake targets are fully wired causes a CMake configuration failure on that intermediate commit. Verify: (1) `aitown_render` target exists, (2) `aitown_ui` target exists, (3) both are listed in `integration_tests` `target_link_libraries` before Phase 3 closes. (ref: `architecture/testing/framework.md`, `architecture/testing/testability-architecture.md`)
...
- `UIManager` event dispatch chain is structurally correct: all 6 priority tiers present in `UIManager::onEvent()`, Priority 2 has both guards; verified by code review AND by `UIManagerModalTest.ScrimBlocksHUDClickWhenModalActive` compile-only stub presence (Phase 6 implements real assertion) before Phase 4 begins
...
- **CRITICAL**: `IAudioSystem.h` includes `simulation_types.h` for `SimSpeed` — verified by `grep '#include.*simulation_types' src/interfaces/IAudioSystem.h` returning a match before Phase 3 closes; omitting this include causes duplicate-type compile errors in translation units that include both `IAudioSystem.h` and `ICitySimulation.h`
- **CRITICAL**: `audio_tests` `target_include_directories` contains all 4 required paths: `tests/simulation/`, `src/interfaces/`, `src/audio/`, and `${CMAKE_SOURCE_DIR}` — verified by CMakeLists.txt review before Phase 3 closes
- **HIGH**: `mock_audio_system.h` contains no `#include` of `src/ui/` or `tests/ui/` headers — verified by `grep '#include.*ui/' tests/simulation/mock_audio_system.h` returning no matches before Phase 3 closes
...
- **RISK** ✅ VERIFIED CLEAR: `UIManager` construction order violated — `NotificationManager` constructed after another panel — causes crash when that panel enqueues a notification during its own construction. **Mitigation**: `UIManagerDrawOrderTest` fixture must verify construction succeeds without crash; code review must verify construction order matches spec before Phase 3 closes.
- **RISK** ✅ VERIFIED CLEAR: `ManualRNG` fixture initialization error (`rng_{}` instead of `rng_{{0}}`) causes all tests in a fixture to fail with a cryptic `std::invalid_argument`. **Mitigation**: document in Phase 3 plan; 6 self-tests catch this pattern early.
- **RISK** ✅ VERIFIED CLEAR: `vec3.h` and `camera_state.h` headers have companion `.cpp` files — causes `ui_tests` link failures if `aitown_sim` not linked. **Mitigation**: verify both are header-only before Phase 3 closes; if non-inline functions needed, add `aitown_sim` to `target_link_libraries(ui_tests ...)`.
