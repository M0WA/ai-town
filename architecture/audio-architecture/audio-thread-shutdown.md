...
3. **Guard join with `joinable()`**: `if (m_audioThread.joinable()) m_audioThread.join();` — always check `joinable()` before calling `join()`. The audio thread is never launched in two cases: (a) `alcOpenDevice` returned null (silent mode — `m_deviceLost=true`, constructor returns early); (b) the constructor threw before reaching the `std::thread` constructor (e.g., `ALC_EXT_thread_local_context` absent). In both cases `m_audioThread` is a default-constructed `std::thread` (not joinable); calling `join()` on it is undefined behavior.
