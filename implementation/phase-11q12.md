...
- [ ] Test `resolveModelPath()` with a real `IFileSystem*` (the primary
  ...
  - When only `foo_lod0.b3d` exists: returns `.b3d` path.
  - Drop the device after the test case (`device->drop()`).
...
- [ ] **Coverage note**: the test file lives in `tests/integration/` (under the
      ...
      five test cases above (three `nullptr` fallback + two `IFileSystem*` path).
...
existing C++ files, ~30 lines changed in `validate_assets.py`. No new
dependencies (only `<filesystem>` added in the `.cpp`, which is already available in
GCC 12+ with `-lstdc++fs` if needed).
...
- [ ] `resolveModelPath()` integration tests pass (all five cases: three
      nullptr-fallback — PLY-present, B3D-only, neither-present — and two
      IFileSystem* via EDT_NULL — PLY-present, B3D-only). These run under
