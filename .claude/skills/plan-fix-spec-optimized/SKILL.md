...
Optimizations over the baseline skill:
- **Round 1 only**: prod-owner syncs the plan; skipped on subsequent rounds
- **Domain-scoped re-review**: only agents whose domain was touched re-run in later rounds
- **Diff-based prompts**: agents receive a change summary rather than re-reading all files
- **Structured issue schema**: agents emit machine-readable issue blocks for dedup
- **Issue deduplication**: overlapping issues merged before fix tracks launch
- **Fix verification**: each fixing agent self-reviews their own changes before the round closes
- **Carry-forward clean status**: agents with NO ISSUES in an untouched domain auto-pass
- **Deferred single commit**: one commit at the final clean pass (not per round)
...
```
=== PLAN FIX SPEC COMPLETE (OPTIMIZED) ===
