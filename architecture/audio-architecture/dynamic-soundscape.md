...
- **Stinger_crisis trigger specification**: `triggerStinger(StingerType::CRISIS)` fires when **Phase 8 UIManager** receives the **second consecutive deficit-month notification** (`getConsecutiveDeficitMonths() == 2`) from its `pollPendingNotification()` polling loop. Specifically: on receipt of a `BudgetDeficitWarn` notification when `getConsecutiveDeficitMonths() >= 2`. Call site: **Phase 8 UIManager only** — `CitySimulation` MUST NOT call `triggerStinger(StingerType::CRISIS)` directly. Cooldown: 5 real seconds minimum between triggers of the same type (standard stinger cooldown). Co-fire rule: `stinger_crisis` fires alongside any `SFX_BUDGET_WARN` sound for the same deficit event — the SFX and stinger are independent audio layers; the stinger ducks music while the SFX plays at normal gain without ducking. `sfx_service_degrade` does NOT trigger `stinger_crisis` (service degradation is an advisory event; crisis is reserved for imminent bankruptcy). `stinger_crisis` fires at most once per deficit streak (minimum-time-between-triggers prevents re-fire until streak resets and re-enters). **Stinger vs SFX decision matrix**: (1) budget deficit crossing −25%: `SFX_BUDGET_WARN` fires (UIManager via audio callback); (2) budget deficit month 2 streak: additionally `stinger_crisis` fires via UIManager; (3) service degradation: `SFX_SERVICE_DEGRADE` fires (CitySimulation audio callback) — no crisis stinger; (4) City Rating transition: `stinger_milestone` fires (UIManager) — no crisis stinger.

- **Adaptive music intensity deferral**: Adaptive music intensity tiers (calm/growth/crisis) based on treasury balance trend, population growth rate, and deficit streak are **Phase 10 deliverables**. Phase 6 delivers only the `getTimeOfDay() → TimeOfDay` accessor; Phase 10 wires `setTimeOfDay()` and adds `setMusicIntensity(MusicIntensity)` to `IAudioSystem` as needed. In V1 prior to Phase 10, music uses a fixed intensity track selection at startup.
