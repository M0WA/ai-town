...
Note: The `validate-assets` job definition and `all-checks-pass` wiring introduced in Phase 1 remain unchanged in Phase 2 (when the stub script gains 13 real asset checks) and Phase 6 (when a 14th sidecar check is added). Only the script content changes across those later phases — the CI wiring is stable from Phase 1 onward.
