...
- **Transient toasts**: Displayed in the right-rail (top:64, right:12, 292 px wide; see Right-Rail Layout table below); CRITICAL band: up to `kNotifMaxVisible` (3) simultaneously; Normal band: up to `kNotifMaxVisible` (3) simultaneously (reduced when CRITICAL band is occupied — see priority separation below). **Auto-dismiss (5 seconds) applies only to Normal queue toasts.** CRITICAL queue toasts do NOT auto-dismiss — they require explicit player dismissal (click, Enter, or Delete).
- **Priority separation**: Toasts are separated into two queues with **dedicated vertical screen bands** to prevent overlap:
  ...
  **Auto-resume disambiguation — `dismissCriticalToast()` vs. `closeModal()`**: These two operations have distinct resume semantics and must not be conflated:
  ...
  - **Simultaneous CRITICAL toast + blocking modal behavior**: When a blocking modal becomes active, **ALL notification cards — CRITICAL and Normal — both currently visible and any new arrivals — are hidden**. Pre-existing visible cards are returned to the front of their respective queues and their on-screen elements removed. New cards arriving while a modal is active are held in their queues without being displayed. After the modal is dismissed, all queued cards become visible (up to `kNotifMaxVisible` (3) simultaneously, CRITICAL cards filling from slot 0) and CRITICAL-toast auto-pause is re-evaluated. This ensures the modal has complete visual focus with no competing blocking UI layers. **Rationale for hiding all cards**: showing any notification card on top of the modal scrim creates competing UI layers, forcing the player to decide which to address first. The modal always takes full precedence. After modal dismissal the player immediately sees any outstanding alerts.
...
- **Panel background**: The log panel has a dark semi-opaque background applied via
  ...
  by `toggleLog()`. This produces the Glacier Glass deep-navy fill (r=4, g=12, b=28) at approximately
