...
- **Adaptive music intensity — Phase 10 deliverable**: Adaptive music intensity tiers (calm/growth/crisis) are **Phase 10 deliverables**. Phase 6 delivers only the `getTimeOfDay() → TimeOfDay` accessor; Phase 10 wires `setTimeOfDay()` and adds `setMusicIntensity(MusicIntensity)` to `IAudioSystem`. In V1 prior to Phase 10, music uses a fixed intensity track selection at startup. The three thresholds are locked in `architecture/game-design/economy-model.md` Music Intensity Tiers section and are final:
  ...
  **Implementation contract for `CitySimulation::update()` wiring**: evaluate all three conditions in priority order each `update()` call and call `audioSystem->setMusicIntensity(tier)` with the result. The `setMusicIntensity()` call is a no-op when the tier is unchanged from the previous call — `AudioSystem` must guard against unnecessary crossfade commands when the tier has not changed. `consecutive_deficit_months` is the existing field subject to the `m_firstRevenueTicked` gate (not incremented before at least one non-zero revenue budget tick). The `budget_surplus_pct` used in the CALM condition is the same value already computed each budget tick for the forced-loan and deficit warning gates.
...
- Deliver a crossfade audibility demo (day→night at 3 s constant-power, no intermediate) as part of the ambient bed delivery package. This demo must be reviewed and approved before ambient bed asset lock. **Delivery artifact**: commit as `assets/audio/crossfade_demo_day_to_night.ogg` — a 10–15 s render starting with ~4 s of the day bed alone, followed by a 3 s constant-power crossfade (gain_out = cos(t×π/2), gain_in = sin(t×π/2)), ending with ~3 s of the night bed alone. Low-fidelity prototypes or early sketches are acceptable; the evaluation criterion is that the 3 s overlap does not produce an audible jarring transient or tonal clash. This file is a QA artifact and is not loaded by `AudioSystem` at runtime.
