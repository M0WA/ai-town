...
| Small buildings / props | 500–1500 tris | 100–300 tris | Billboard |
...
| `height_floors` | integer | yes | Total floor count. Used by export validation check #2 (billboard threshold ≤ 3 floors), check #11 (geometry shell requirement > 3 floors), and the C++ `LODNode` runtime upgrade path (billboard ↔ geometry shell switch on density tier change). Also used to compute building height (`height_floors × 3 m`) for collision mesh extrusion. |
...
- **Small Building / Prop LOD2 (Billboard) Asset Contract**:
  Small buildings and props use the billboard imposter system at LOD2. These assets must **NOT** ship a `_lod2.b3d` file. Instead they require:
  - A pre-baked imposter atlas named `<asset_name>_billboard.dds` (see `2d-texture-standards.md — Billboard Imposter Atlas` for the full format, mip chain, and sRGB upload path specification).
  - The C++ `LODNode` for the small building category loads `_lod0.b3d`, `_lod1.b3d`, and `_billboard.dds`. No `_lod2.b3d` is loaded or expected.
  - The export validation script must flag any small building or prop asset that has a `_lod2.b3d` file as an error — small buildings and props must use the billboard system, not a geometry LOD2 shell.
  - Large buildings use `_lod2.b3d` (geometry shell). The validation script distinguishes asset categories via a metadata file (`<asset_name>.meta`) that specifies `category: large_building | small_building | prop`.
...
- **Per-module polygon caps**:
  ...
  **Export validation script — required checks**: The export validation script (`tools/validate_assets.py` or equivalent) must perform all 14 required checks and produce a per-asset PASS/FAIL report:
  ...
  2. Small building / prop `_lod2.b3d` file is absent — **conditional**: only flag missing billboard if `height_floors ≤ 3` (read from `<asset_name>.meta`). If `height_floors > 3`, the asset must have a `_lod2.b3d` geometry shell INSTEAD OF (or in addition to) the billboard; a `_lod2.b3d` file is then REQUIRED (see check #11).
  ...
  14. `.meta` sidecar file presence — every `.b3d` building or vehicle file must have a corresponding `<asset_name>.meta` sidecar file. Missing sidecar: validation error. This check must be present in the Phase 2 `validate_assets.py` stub and active from Phase 2 onward.
