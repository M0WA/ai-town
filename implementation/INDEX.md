...
| [1](phase-1.md) | Render Skeleton & Camera | `RenderSystem` RAII (`EDT_OPENGL`, `Bits=32`, `Vsync=false`), render loop order (`UIManager::draw()` — not `m_gui->drawAll()`), GL capability queries (consolidated EDT_NULL guard), `IAudioSystem.h` (11 methods, `SoundPriority`; includes `simulation_types.h` for SimSpeed), `audio_types.h` (includes `vec3.h`/`camera_state.h`; no SimSpeed), `sound_ids.h` (all V1 SoundId + MusicTrackId constants), `ICitySimulation.h` (+ `getTotalPopulation()`; Difficulty PascalCase from `simulation_types.h`), `IUIBackend` (14 methods in `src/ui/`), `UIManager` shell (4-param constructor with IClock*; `transitionToGameplay(GameMode)`; `showSettings()` stub; 10 draw-order slots wired; LoanTerms.h + ui_types.h includes), HUD stub (`IUIBackend*` + `IClock*` + `ICitySimulation*`), `UIScaler` (6-param), `CameraController` in `src/ui/` (`InputEvent` with `WindowFocusGained`/`WindowFocusLost`), `TextureCache` skeleton (3 pools: linear/sRGB/splatMap; all stubs), GLSL stubs, `Irrlicht.dll` Windows post-build copy + CI hard-fail, `ManualClock`/`ManualRNG`/`WallClock.cpp`/`NullSimulationPauser`/`MockSimulationPauser`, `ui_tests` (5-path includes + RapidCheck) + `integration_tests` (full link + include config), `tools/vehicle_atlas_registry.json` stub (recovered from Phase 0), bootstrap oscillation design review gate | `graphics-dev-irrlicht`, `gamedesign-ux`, `sound-dev-opensoftal`, `test-dev-cpp`, `gamedesign-lookandfeel`, `graphics-artist-3d-model`, `graphics-artist-2d-texture`, `cicd-dev-github` | Planned |
