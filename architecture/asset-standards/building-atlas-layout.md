...
| 3 | 2 | service_buildings | All service types (fire_station, police_station, power_plant, water_tower) | N/A — infrastructure | Shared across all four V1 service building types. All share a common palette (concrete, glass, utility panels). Assigned in `architecture/asset-standards/3d-model-standards.md` Service Building Model Standards. Confirmation by `graphics-artist-2d-texture` required before Phase 9 service building UV authoring begins — see sign-off gate below. |
| 3 | 3 | RESERVED | — | — | Available for Phase 9+ service building material variants. Do not assign in V1. |

**Note**: Cells (0,0)–(3,1) are PLACEHOLDERS with module-type identities confirmed but texel content not yet authored — content is assigned during Phase 9 UV authoring. Cell (3,2) is now assigned to service buildings per `3d-model-standards.md` Service Building Model Standards. Cell (3,3) remains reserved for Phase 9+ use. Update the placeholder cells (0,0)–(3,1) with final texel assignments before Phase 9 UV authoring begins; the service building cell (3,2) assignment is binding and must not be reallocated.
...
- [ ] All V1 module variant types have a cell assignment (no unbounded "assign in Phase 9" placeholders remaining for zone building module types — service building cell (3,2) is already assigned)
...
- [ ] Document reviewed and approved by `graphics-artist-2d-texture`; this approval must explicitly confirm that service building cell (3,2) (concrete, glass, utility panels — 496×496 px usable area) is sufficient for all four V1 service building types sharing a common material palette
...
<!-- PENDING SIGN-OFF: graphics-artist-2d-texture — service building atlas cell (3,2) content confirmation REQUIRED before Phase 9 service building UV authoring begins. Required confirmation: (a) a shared palette covering concrete, glass, and utility panel materials for all four V1 service building types fits within the 496x496 px usable area of cell (3,2) at the target texel density; (b) the second reserved cell (3,3) is not needed for V1 service buildings and remains available for Phase 9+ use. Until this confirmation is recorded, service building UV authoring in Phase 9 must treat cell (3,2) as provisionally assigned pending 2d-texture artist review. This does NOT block building mesh production (LOD0/LOD1/LOD2 geometry and collision meshes); it blocks only service building UV channel 0 authoring and the validate_assets.py check #4 UV-range verification for service building assets. -->
