...
- [ ] **Add HUD panel-background elements for the 5 HUD panels**: In `HUD.cpp` constructor,
      ...
      > establish layout; D13 then upgrades to rounded-corner rendering. These 5 HUD
      > panel-background handles are **intentional temporary scaffolding** — do NOT
      > write standalone unit tests for them; D9 tests target the final D13 rendering
      > path only.
...
- [ ] `drawOverlay()` iterates visible cards and draws the 3 px left severity strip
      and the 2 px chrome top rim (§1 above) for each via `m_backend->fillColoredRect(...)`.
      Neither strip is a child element.
...
- [ ] `NotificationManager.h` private members: the
      ...
      **IHUDBadgeNotifier injection**: Add a fifth nullable constructor parameter
      ...
      `m_badge` private member (`IHUDBadgeNotifier* m_badge{nullptr}`). Call
      `if (m_badge) m_badge->incrementNotificationBadge();` on the following disposal
      paths: (1) slot-overflow displacement §(a) collapse, (2) player-dismiss via
      `dismissCard()`, and (3) auto-dismiss via `update()` timer expiry. Do NOT call
      on §(b) queue-capacity eviction (the card was never displayed to the player —
      see `.Times(0)` assertion in `NotifRail_EvictedEleventhNormal_AppendsToLog`).
...
In the `HUD` constructor, immediately after the minimap element is created:
    ...
    m_btnMinimapMap     = m_backend->addButton("", 1720, 838, 64, 22);
    m_btnMinimapTraffic = m_backend->addButton("", 1788, 838, 64, 22);
    m_btnMinimapService = m_backend->addButton("", 1856, 838, 64, 22);
...
Per-button text colour is delivered via child `IGUIStaticText` label overlays on each
minimap toggle button. `IGUIButton::setOverrideColor()` does NOT exist in the vcpkg
Irrlicht port, and `IGUISkin::setColor(EGDC_BUTTON_TEXT, ...)` is a global setting
affecting all buttons — so the only way to get per-button text colour is a child
`IGUIStaticText` element per button whose text colour is set independently.

Add three additional `UIElementHandle` members to `HUD.h`:

    UIElementHandle m_lblMinimapMap{kInvalidUIElement};
    UIElementHandle m_lblMinimapTraffic{kInvalidUIElement};
    UIElementHandle m_lblMinimapService{kInvalidUIElement};

In the `HUD` constructor, immediately after the three `addButton` calls, create child
`IGUIStaticText` overlays with empty-label buttons (set button text to `""` on creation):

    // Child IGUIStaticText overlays for per-button text colour.
    // Buttons are created with empty labels ("") — text is drawn by the child overlay.
    m_lblMinimapMap     = m_backend->addStaticText("MAP",     1720, 838, 64, 22);
    m_lblMinimapTraffic = m_backend->addStaticText("TRAFFIC", 1788, 838, 64, 22);
    m_lblMinimapService = m_backend->addStaticText("SERVICE", 1856, 838, 64, 22);

    // Active button: #60C8E8 = rgb(96,200,232) fully opaque
    m_backend->setElementTextColor(m_lblMinimapMap, 96, 200, 232);
    // Inactive buttons: rgba(140,180,220,0.55) = SColor(140, 140, 180, 220)
    m_backend->setElementTextColorA(m_lblMinimapTraffic, 140, 180, 220, 140);
    m_backend->setElementTextColorA(m_lblMinimapService, 140, 180, 220, 140);

In `onMinimapOverlayClick(UIElementHandle clicked)`, after switching sprites, update
text colours on the child label overlays (NOT on the button handles):

    // Active: #60C8E8 fully opaque; Inactive: rgba(140,180,220,0.55)
    auto setActive   = [&](UIElementHandle lbl) { m_backend->setElementTextColor(lbl, 96, 200, 232); };
    auto setInactive = [&](UIElementHandle lbl) { m_backend->setElementTextColorA(lbl, 140, 180, 220, 140); };

    setInactive(m_lblMinimapMap);
    setInactive(m_lblMinimapTraffic);
    setInactive(m_lblMinimapService);

    if (clicked == m_btnMinimapMap)          setActive(m_lblMinimapMap);
    else if (clicked == m_btnMinimapTraffic) setActive(m_lblMinimapTraffic);
    else if (clicked == m_btnMinimapService) setActive(m_lblMinimapService);

Do **NOT** call `setElementTextColor` / `setElementTextColorA` on the button handles
themselves — those calls route through the `IrrlichtUIBackend` skin-global
`EGDC_BUTTON_TEXT` path and change all button text simultaneously.
...
- [ ] **Legend panel visibility wiring**: After `HUD::onMinimapOverlayClick(clicked)` returns,
      **UIManager** (not HUD) calls `m_minimap->setOverlayActive(clicked != m_hud->getBtnMinimapMap())`
      inside `UIManager::onEvent()`, where the `m_minimap` pointer is accessible. HUD exposes
      `UIElementHandle getBtnMinimapMap() const` (inline getter) so UIManager can compare.
      `Minimap::m_overlayActive` reflects the current mode. `Minimap::m_overlayActive`
...
- [ ] **Dot indicators and toggle button typography**: The 5 px diameter filled-circle dot
    ...
    Toggle Appearance` are drawn per-frame in `UIManager::drawOverlays()` via
      ...
      label text ("MAP", "TRAFFIC", "SERVICE") is used for toggle button labels (rendered by
      child `addStaticText()` overlays, NOT passed to `addButton()`). Letter-spacing is not
      supported by Irrlicht (V1 engine limitation).
...
- [ ] Text label fallback: buttons are created with empty labels `""` because text is
      rendered by child `IGUIStaticText` label overlays (`m_lblMinimapMap/Traffic/Service`)
      which support per-button text colour. The child overlays use uppercase text
      `"MAP"`, `"TRAFFIC"`, `"SERVICE"` (Irrlicht has no `text-transform` equivalent —
      only the literal string is rendered). When `m_spriteTextureReady == false` (sprite
      sheet absent), the child `IGUIStaticText` overlays still render the uppercase text
      as the visible fallback.
...
- [ ] **`UIManager::drawOverlays()` sub-ordering (step 10b)**: The `drawOverlays()`
      ...
      0. **Panel rounded-corner backgrounds via `drawNineSlice`** (added by Deliverable 13) —
         draw 9-slice panel backgrounds for all 7 panels (top bar, toolbar, Zone, Utilities,
         Grace Period, minimap, notification cards). These MUST render first so all subsequent
         foreground elements (button washes, overlays, strips) paint on top of the panel fill.
...
- [ ] Register the new test file in `CMakeLists.txt`:
      ...
      Note: the 2 `HUDGlacierGlass_SetElementTextColorA_*` tests live in `NotificationRailTest.cpp` (Deliverable 11), not in this file.
...
- [ ] Update `architecture/ui-ux/hud-layout.md §Toggle button labels` to state buttons are
      created with empty labels (`addButton("", ...)`) and uppercase strings are passed to
      child `addStaticText()` overlays, not to `addButton()` — matching the child
      `IGUIStaticText` overlay approach used throughout Phase 11q13.
...
- [ ] Update `architecture/ui-ux/notification-system.md §Simultaneous CRITICAL toast + blocking modal behavior` to state that ALL visible cards (CRITICAL and Normal) are hidden when a blocking modal is active — not just CRITICAL toasts — since Phase 11q13's shared 3-slot rail means both severity tiers occupy the same visual space.
- [ ] Update `architecture/ui-ux/minimap.md §Overlay Toggle Buttons`: replace old 32×32 px
      ...
      (kMapY=864, kMapH=200).
- [ ] Update `architecture/ui-ux/minimap.md` overlay toggle parent bullet: replace
      'two icon buttons plus the implicit off state' with 'three toggle buttons
      (Map, Traffic, Service) with mutual-exclusivity' to match the Phase 11q13
      3-button design.
- [ ] Add `void dismissCard(int slotIndex)` to `§NotificationManager API` with contract:
      ...
      `refreshVisibleSlots()` for Normal slots. Before removing elements, appends the
      card's data (title, message, timestamp, severity) to `NotificationLog::append(...)`.
      All disposal paths (player-dismiss, auto-dismiss, collapse, eviction) must log to
      ensure the "last 50 notifications" log contract (`notification-system.md`
      §Notification Log Panel line 126) is honoured. Badge-increment rules are
      path-specific: player-dismiss and auto-dismiss call
      `m_badge->incrementNotificationBadge()`; §(a) collapse (Deliverable 3) also
      badge-increments; §(b) queue-capacity eviction does NOT badge-increment (the card
      was never displayed — see Deliverable 3 §(b) `.Times(0)` assertion).
...
- [ ] Explicitly update notification log panel background in notification-system.md to Glacier Glass deep-navy at higher opacity for dense-text readability: `setElementBackground(h, 4, 12, 28, 217)` (`kLogPanelBg = SColor(217, 4, 12, 28) = 0xD9040C1C`, ~85% opacity). Add `constexpr uint32_t kLogPanelBg = 0xD9040C1Cu;` to `ui_constants.h`. The log panel requires higher opacity than `kGlacierPanelBg` (alpha=66) because it displays a scrollable 50-entry text list against the 3D city view.
- [ ] **Update `CLAUDE.md` §IUIBackend methods bullet**: Replace `dismissCriticalToast(UIElementHandle)` with `dismissCard(int slotIndex)` as the production dismiss API.
...
- [ ] `NotifRail_DismissCard_ShiftsRemainingUp`: After `postNormal` × 2, `dismissCard(0)`
      ...
      old Y coordinates. Also assert
      `EXPECT_CALL(badge_, incrementNotificationBadge()).Times(1)` — player-dismiss
      badge-increments per Deliverable 10 contract.
...
- [ ] `NotifRail_NormalCard_AutoDismiss_AfterFiveSeconds`: Post one Normal card via
      ...
      (b) `notifications_->getNormalQueueSize() == 0` confirms the Normal queue is
          empty after expiry.
      (c) `getLog().getEntryCount() == 1` confirms the auto-dismissed card was appended
          to the notification log (all disposal paths log per Deliverable 10 contract).
      (d) `EXPECT_CALL(badge_, incrementNotificationBadge()).Times(1)` — auto-dismiss
          badge-increments (the card was displayed to the player).
...
- [ ] Replace `setElementBackground` panel-background calls for all 7 panels (top bar,
      ...
      **Draw-order note**: these `drawNineSlice` calls correspond to step 0 of the
      `drawOverlays()` 6-step sub-ordering defined in Deliverable 4; they MUST execute
      before all other overlay draw steps so foreground elements render on top.
...
- [ ] In `HUD::update(float dt)` (called each frame before `drawAll()`):
      ...
      Call `m_backend->setElementAlpha(overlay, t * targetAlpha)` where `targetAlpha`
      is a `float` in [0.0, 1.0] (matching the `IUIBackend` spec signature).
- [ ] In `UIManager::onEvent()` mouse-move handler: hit-test each button rect via
      `getElementRect()`; call `HUD::setHoverState(UIElementHandle, bool)` to update
      the `ButtonHoverState::hovering` flag. HUD itself has no `onEvent()` method
      (Deliverable 4); UIManager owns all click/hover routing. This replaces the existing discrete sprite-swap for hover state.
...
| `src/rendering/IrrlichtUIBackend.h`                                             | Add override declarations for `setElementTextColorA` (method 23) and `drawNineSlice` (method 24, Deliverable 13)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `CLAUDE.md`                                                                     | In the §IUIBackend methods bullet, replace `dismissCriticalToast(UIElementHandle)` with `dismissCard(int slotIndex)` as the production dismiss API. |
