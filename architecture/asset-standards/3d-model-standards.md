...
<!-- SIGN-OFF: graphics-artist-3d-model 2026-02-27 — confirmed: all 20 export validation checks present and correct; naming convention <type>_<zone>_<tier>_lod<N>.b3d; pivot at base center Y=0; 5 mm Y-axis vertical extent tolerance (max vertex deviation from Y=0 bottom or Y=3.0 top per floor module); 10-floor hard cap; collision mesh dispatch order confirmed — (1) _col_0.obj: multi-convex set, (2) _col_circle.obj: N-sided circular prism, (3) _col.obj: single convex hull, (4) none: log error; dispatch prevents _col_0 shadowing _col on dual-suffixed assets; billboard floor count limit (height_floors <= 3 uses billboard imposter, >= 4 uses _lod2.b3d); LOD2 pivot conformance (base-center identical to LOD0/LOD1); atlas mip chain clamped at 4 levels; Blender export axis (-Z Forward, Y Up); asset formats .b3d (animated/rigged), .obj (collision/static). Phase 9 may proceed. -->
