...
- [ ] **CI-5 `actions/setup-python` SHA resolution**: `actions/setup-python` must be SHA-pinned to a real 40-character verified SHA. Resolve at implementation time via `gh release view actions/setup-python --json tagName,assets` or equivalent — do NOT use a placeholder or short SHA. The supply-chain lint step introduced in Phase 0 will fail on any unresolved placeholder. This SHA resolution must be done by `cicd-dev-github` before the `validate-assets` job (which uses `actions/setup-python`) is merged. (ref: `architecture/ci-cd/github-actions-workflow.md`)
