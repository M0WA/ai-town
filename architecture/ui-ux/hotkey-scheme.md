...
- **Undo (Ctrl+Z)**: Modifier chord. Processed by `IEventReceiver` before other handlers. In V1, Ctrl+Z is **not rebindable** (chord handling requires separate key-mapping infrastructure). It appears in the rebinding UI as a non-rebindable informational row: "Ctrl+Z — Undo (not rebindable in V1)". The JSON config format for chord bindings: `"Ctrl+KeyZ"` (modifier prefix + SDL2-style key name). Conflict detection does not apply to non-rebindable chords.
