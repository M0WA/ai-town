...
- [x] Job added before all build jobs in `.github/workflows/ci.yml`.
- [x] Runs on `ubuntu-latest` with no container.
- [x] Steps:
...
- [x] No `needs:` on this job (runs immediately on push/PR).
...
- [x] `needs: [supply-chain-lint]` added to `validate-assets` (ensures SHA lint passes
  before any asset work begins; no throughput cost — validate-assets is fast).
- [x] Steps added to `validate-assets` (in order, after existing steps):
...
- [x] Note: `pip install Pillow` / `pip install mutagen` already present in the job
  (from existing check_21–#23 steps) — no new pip step needed for the above.

#### 3. `build-linux` changes

- [x] `needs:` updated to `[supply-chain-lint, validate-assets]` (appended to any existing
  `needs:` entries).
- [x] Removed steps:
...
- [x] Step number comments in the job updated so there are no gaps.

#### 4. `build-windows` changes

- [x] `needs:` updated to `[supply-chain-lint, validate-assets]`.
- [x] Removed steps:
  - "Lint workflow for placeholder SHAs" (supply-chain lint).
...
- [x] `needs:` updated to `[supply-chain-lint, validate-assets]`.
- [x] Removed steps (identical list to `build-linux`):
...
- [x] `supply-chain-lint` added to `needs:` list.
- [x] `if: always()` condition preserved (required for branch protection to work on
  cancelled runs — do not remove).
- [x] Note: `validate-assets` is already in `all-checks-pass` `needs:` — no change
...
- [x] `supply-chain-lint` job exists; no duplicated SHA lint step in any build job.
- [x] `validate-assets` gains `needs: [supply-chain-lint]` and the five new presence
  steps (clouds, fonts, audio, terrain, vcpkg baseline); existing check_N steps
  and Python setup are preserved unchanged.
- [x] `build-linux` has `needs: [supply-chain-lint, validate-assets]` and zero
  duplicated preflight steps (including the post-build font duplicate).
- [x] `build-windows` has `needs: [supply-chain-lint, validate-assets]` and zero
  duplicated preflight steps (including `pip install Pillow` and post-build font
  duplicate).
- [x] `coverage-linux` has `needs: [supply-chain-lint, validate-assets]` and zero
  duplicated preflight steps (including the post-build font duplicate).
- [x] `all-checks-pass` includes `supply-chain-lint` in its `needs:` list and retains
  `if: always()`.
- [x] Eight reusable workflow files exist under `.github/workflows/` with `_` prefix,
  each declared with `on: workflow_call:` and the correct `inputs:` block.
- [x] `ci.yml` contains no inline `steps:` for any job except `all-checks-pass`.
- [x] Supply-chain lint in `_supply-chain-lint.yml` greps all `*.yml` files in
  `.github/workflows/` (not just `ci.yml`).
- [x] CI passes: all build jobs run, coverage gate enforced, `all-checks-pass` green.
- [x] No changes to CMakeLists.txt, source files, Dockerfiles, `vcpkg.json`,
...
| Role | Area | Status |
|---|---|---|
| `cicd-dev-github` | supply-chain-lint, validate-assets extension, needs wiring, reusable workflow split, gate job | ✅ `.github/workflows/_supply-chain-lint.yml`, `_validate-assets.yml`, `_build-linux.yml`, `_build-windows.yml`, `_coverage-linux.yml`, `_markdown-lint.yml`, `_package-windows.yml`, `_package-linux-deb.yml` created; `ci.yml` is now a thin orchestrator with no inline steps except `all-checks-pass` |
