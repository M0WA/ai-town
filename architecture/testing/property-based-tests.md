...
- **Zero-revenue surplus tests** (unit tests in `tests/simulation/`): These tests exercise a deterministic code path (zero revenue → no deficit consequences) and must use `StrictMock<MockAudioSystem>` and `StrictMock<MockRenderer>` — NOT `ManualRNG`. `ManualRNG` is only needed for tests that exercise random building selection (service degradation). Zero-revenue tests have no random draws; injecting ManualRNG would be misleading (it implies randomness is involved). Using StrictMock ensures that no unexpected audio/render calls fire during the zero-revenue path. Required `TEST_F` cases:
  ...
  4. `GameOverThreshold_RequiresConsecutiveTicks`: Explicitly verifies the consecutive-month requirement for game-over. Use `monthly_revenue = $1,000`, `total_monthly_expenses = $3,000` (−200% surplus, well below the −50% threshold). Run ONE budget tick → verify `getConsecutiveDeficitMonths() == 1` (streak = 1, not yet 3). Run a SECOND consecutive tick → verify `getConsecutiveDeficitMonths() == 2` (streak = 2). Run a THIRD consecutive tick → verify `getConsecutiveDeficitMonths() == 3` (game-over condition met). The game-over condition is `getConsecutiveDeficitMonths() >= 3` AND `budget_surplus_pct <= -50%`. **Do NOT add `isGameOverSurplusThresholdBreached()` to ICitySimulation** — this is not an interface method; all test assertions must use the existing `getConsecutiveDeficitMonths()` accessor, which is already on `ICitySimulation`. Splitting this from test 3 makes the consecutive-state-machine logic independently verifiable.
...
- **Power grid service coverage unit tests** (in `tests/simulation/`, pure C++ logic):
  ...
  2. `PowerCoverage_ZeroTilesInRange_ReturnsNASentinel`: a power plant with no buildable tiles reachable returns `ServiceCoverage::power == -1.0f` (N/A sentinel per `service-coverage.md` zero-tiles-in-range guard — do NOT return 100%; the Inspector panel displays "N/A — no served zones in range").
