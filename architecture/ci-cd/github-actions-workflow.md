...
**Scope of this lint step**: This lint step is placed in `build-linux` ONLY. Since all jobs share the same `ci.yml` file checked out from the same commit, a lint failure in `build-linux` blocks the `all-checks-pass` gate and prevents merging — duplicating this step in `build-windows` and `coverage-linux` provides no additional security guarantee beyond what `build-linux` already enforces. References to this lint "failing in `build-linux`/`build-windows`/`coverage-linux`" describe the gate effect: when `build-linux` fails, `all-checks-pass` exits non-zero regardless of the other jobs' outcomes.
