...
- VRAM size estimated per texture format (format-aware, block-based calculation):
  ...
  - **Note on the 1.33× overhead factor**: this factor applies equally to **4-level and
    5-level mip chains** — the additional fifth mip level (e.g. 256×256 for a 4096×4096
    atlas) adds only ~0.4% of the mip-0 footprint and is negligible for budget estimation
    purposes.
...
- **sRGB upload path** — **fully raw OpenGL only**: `addTexture(ECF_A8R8G8B8)` + `glCompressedTexImage2D` is **invalid** — `addTexture` allocates the GL object with a linear internal format already committed; `glCompressedTexImage2D` on that object produces `GL_INVALID_OPERATION`. The `addTexture` + `COpenGLTexture::getOpenGLTextureName()` approach has the same root cause. The only correct path:
  ...
  **Wrap mode requirement**: Billboard atlas textures (filename suffix `_billboard`) MUST have
  ...
  sRGB atlas textures use the default `GL_REPEAT` (road markings, building facade). The Phase 2
  stub body is a no-op; this is a Phase 5 implementation requirement that must be documented in
  Phase 2.
...
| Road marking atlas (sRGB DXT5) | `_road_marking` | `loadSRGB()` | `GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT` (from FourCC) | **3** | Road marking atlas (`road_marking_atlas.dds`) contains diffuse road surface color and lane marking color — visual data requiring gamma-correct sampling. Upload via the sRGB path, consistent with `2d-texture-standards.md §Road marking`. |
...
**`despawnVehicleAgent` does NOT call `evictUnreferenced()`**: Vehicle atlas textures (`vehicles_diffuse_atlas_d.png`, `vehicles_normal_atlas_n.dds`, `vehicles_sprite_atlas_d.dds`) are loaded **once at renderer init** via `IVideoDriver::getTexture()`, not through `TextureCache`. Because `despawnVehicleAgent` bypasses `SceneEntityManager` entirely, it does not hold a `TextureCache*` reference and does not need to call `evictUnreferenced()` (Step 3 of the `SceneEntityManager` destroy sequence). The texture-slot clear loop in `despawnVehicleAgent` (Steps 3–4: `mat.setTexture(t, nullptr)` + `driver->setMaterial({})`) correctly releases the node's texture references without disturbing the shared atlas. See `architecture/graphics-architecture/scene-graph-ownership.md §Agent Vehicle Node Registry — despawnVehicleAgent` for the full eviction sequence.
...
- **Eviction safety**: Before any entity destruction, `SceneEntityManager::destroy()` must iterate over every material slot on the scene node and zero the texture layers explicitly (clearing the node's own material array), AND call `IVideoDriver::setMaterial(SMaterial{})` (to flush the driver's last-bound state). Both steps are required:
  ...
  **`TextureCache` exposes release paths per pool** — use the correct one based on how the texture was created. **All release methods only decrement ref_count; they do NOT immediately delete the texture object.** The actual deletion of GL resources happens exclusively in `evictUnreferenced()`. This separation prevents double-free: if the same texture is bound to multiple material slots, multiple `releaseLinear()` calls correctly decrement ref_count step-by-step until it reaches 0, at which point `evictUnreferenced()` performs the single deletion.
  ...
  - **`TextureCache::releaseSRGB(const std::string& filename)`**: releases an sRGB diffuse texture from the raw-GL pool. Decrements `ref_count`. **Does NOT call `glDeleteTextures()` immediately** — zero-ref sRGB entries remain until `evictUnreferenced()` is called. **Phase 2 stub requirement**: `releaseSRGB(const std::string& filename)` must be present in the Phase 2 `TextureCache` stub header as a no-op method. `SceneEntityManager::destroy()` calls this in Step 1b — if the method is missing from the Phase 2 header, Phase 5 `SceneEntityManager::destroy()` will fail to compile.
  - **`TextureCache::releaseSplatMap(const std::string& filename)`**: releases a splat map entry from the splat map pool. Decrements `ref_count`. Does NOT call `glDeleteTextures()` immediately — zero-ref entries remain until `evictUnreferenced()` is called. **Phase 2 stub requirement**: `releaseSplatMap(const std::string& filename)` must be present in the Phase 2 `TextureCache` stub header as a no-op method. `SceneEntityManager::destroy()` calls this in Step 1c — if the method is missing from the Phase 2 header, Phase 5 `SceneEntityManager::destroy()` will fail to compile.
  ...
  **CRITICAL constraint — `evictUnreferenced()` call-site safety rule**: `evictUnreferenced()` MUST only be called during the game-logic update phase — strictly before `driver->beginScene()` is called. It MUST NOT be called from within any Irrlicht scene callback, event handler, or any code path that may execute inside `sceneManager->drawAll()`. Violations will cause use-after-free on scene nodes whose textures are evicted mid-draw. In particular, `evictUnreferenced()` must NOT be called from within `OnSetConstants()`: if eviction fires while a draw call is in progress (inside the shader callback), `glDeleteTextures` would delete a texture that may still be referenced by the current draw command's parameter snapshot. `evictUnreferenced()` is called from `SceneEntityManager::destroy()` during the game logic update phase — before `beginScene()` and NOT within any `drawAll()` call. See `irrlicht-device-lifecycle.md §Per-Frame Loop` for the canonical combined frame sequence and the eviction ordering constraint. Without calling `releaseLinear()` in Step 1, the ref_count is never decremented and `evictUnreferenced()` in Step 3 is always a no-op, causing texture memory to leak.
...
### evictUnreferenced() Contract

`evictUnreferenced()` MUST only be called during the game-logic update phase — strictly
before `driver->beginScene()` is called. It MUST NOT be called from within any Irrlicht
scene callback, event handler, or any code path that may execute inside
`sceneManager->drawAll()`. Violations will cause use-after-free on scene nodes whose
textures are evicted mid-draw.
