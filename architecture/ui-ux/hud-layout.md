...
- Primary toolbar with tool modes: Zone, Road, Utilities, Demolish, Query — **left panel**, virtual bounds: x: **8–72 px** (8 px left margin so the toolbar does not clip at the window edge on non-16:9 resolutions and ultrawide aspect ratios), y: 64–600 px (below resource bar, above minimap area). **Note: the tool icon group occupies y:64–600 px. The full left-panel input carve-out region (as defined in `input-arbitration.md` Priority 3 and `ui_constants.h` as `kToolbarBottom=784`) spans y:64–784 — covering all left-panel elements including the undo button, demand bars, and active tool indicator. `kToolbarBottom=784` must be used for the carve-out, not 600.** Each tool mode is a 48×48 px icon button with 8 px padding. Active tool highlighted with accent-color border.
