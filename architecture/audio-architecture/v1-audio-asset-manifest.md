...
|---|---|---|---|
| TBD | `music_*.ogg`, `ambient_*.ogg` | Sample rate must be 44100 Hz; channel count must be stereo (2 channels) for all music stems (`music_*.ogg`) and all ambient beds (`ambient_*.ogg`). **This stereo check covers ONLY files matching `music_*.ogg` and `ambient_*.ogg` — it does NOT extend to zone loops (`sfx_zone_*.ogg`). Zone loops are mono (1 channel) and are validated separately in the `sfx_zone_*.ogg` row below. Phase 5 implementers MUST NOT apply this stereo check to any file matching `sfx_zone_*.ogg`.** | Any file where `vi->rate != 44100` or `vi->channels != 2`; authoring at any other sample rate is a hard asset error |
