---
name: fix-design-implementation
description: Iteratively fix all CRITICAL/HIGH issues in both specs and the implementation plan until a clean pass is achieved — using only the design squad (Game Designer, UI/UX Designer, 2D Texture Artist, 3D Model Artist, Sound Artist). Domain-scoped re-reviews, diff-based prompts, issue deduplication, fix verification, and deferred commits. Does NOT sync the plan from specs first.
---

# Fix Design Implementation

Iteratively review and fix issues across both the spec files and the implementation plan —
repeating until a full clean pass is achieved on the targeted phases. Only the design squad
agents are used. The implementation plan is taken as-is; no Product Owner sync step is run.
...
**Round 1**: run all 5 design squad agents (full read, no diff available yet).
...
| Domain | Spec files |
|---|---|
| `gamedesign-lookandfeel` | `architecture/game-design/` |
| `gamedesign-ux` | `architecture/ui-ux/` |
| `graphics-artist-2d-texture` | `architecture/asset-standards/2d-texture-standards.md`, `architecture/asset-standards/building-atlas-layout.md` |
| `graphics-artist-3d-model` | `architecture/asset-standards/3d-model-standards.md`, `architecture/asset-standards/building-atlas-layout.md` |
| `sound-artist-opensoftal` | `architecture/audio-architecture/` |
...
> domain: [your agent type, e.g. sound-artist-opensoftal]
...
```
=== DESIGN IMPLEMENTATION REVIEW — Round N ===
Severity filter: [TARGET_SEVERITIES]
Review model:    [TARGET_MODEL]
Agents run: X / 5  (skipped: [list of skipped agents and reason])
...
**Did every design squad agent (all 5) reach `CLEAN_DOMAINS` this round?**
...
Re-run **all 5 design squad agents** simultaneously using the **Round 1 prompt** (full read, no
diff) and **`model: [TARGET_MODEL]`**. This is an independent re-review from scratch — same
prompt as Round 1 but on the now-fixed files.
...
- If **yes** → all 5 agents confirmed clean on a fresh read; proceed to Step 6.
...
git add -A
git commit -m "fix(specs+plan): apply plan+spec fixes from design squad review (N rounds)"
```
...
```
=== FIX DESIGN IMPLEMENTATION COMPLETE ===
...
by all 5 design squad agents with no [TARGET_SEVERITIES] issues remaining.
```
...
- Only the 5 design squad agents are used (no tech squad agents)
- All 5 agents run on round 1; later rounds are domain-scoped (Step 1)
...
- A "clean pass" only counts as done after the Step 5a verification pass (all 5 agents re-run from scratch) also returns `NO ISSUES FOUND` — if the verification pass finds issues, fix cycles continue
