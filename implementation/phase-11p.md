...
- [x] **MM-01** `src/interfaces/IUIBackend.h`: Add pure-virtual method 22
...
- [x] **MM-02** `src/rendering/IrrlichtUIBackend.h` + `IrrlichtUIBackend.cpp`: Implement
...
- [x] **MM-03** `tests/ui/MockUIBackend.h`: Add `MOCK_METHOD(void, fillColoredRect,
  (int, int, int, int, int, int, int, int), (override))` stub.
  (ref: `architecture/testing/testability-architecture.md` — `IUIBackend` mock policy)

- [x] **MM-04** Any `StubUIBackend` used in smoke tests (e.g., `ui_smoke_test.cpp`): Add a
...
- [x] **MM-05** `src/interfaces/camera_state.h`: Add three new fields to the `CameraState`
...
- [x] **MM-06** `src/ui/CameraController.cpp`: Populate the three new fields in
...
- [x] **MM-07** `src/ui/Minimap.h`: Fix the header comment that describes Industrial zone
  colour as "orange" — it must read "yellow" to match `architecture/ui-ux/minimap.md`
  (I=yellow, `#F39C12`).

- [x] **MM-08** `src/ui/Minimap.h`: Add public method
...
- [x] **MM-09** `src/ui/Minimap.h`: Add public method
...
- [x] **MM-09a** `src/ui/Minimap.h`: Add public method
...
- [x] **MM-10** `src/ui/Minimap.h`: Add public method
...
- [x] **MM-11** `src/ui/Minimap.h`: Replace `UIElementHandle m_toggleBtn` with two named
...
- [x] **MM-12** `src/ui/Minimap.h`: Add private member `UIElementHandle m_labelStrip{kInvalidUIElement}`
...
- [x] **MM-13** `src/ui/Minimap.h`: Add private members `CameraState m_cameraState{}`,
...
- [x] **MM-13b** `src/ui/Minimap.h` + `src/ui/Minimap.cpp` + `src/ui/UIManager.cpp`: Widen
...
- [x] **MM-13c** `tests/ui/minimap_overlay_test.cpp` — Migrate the existing Phase 11d
...
- [x] **MM-14** `src/ui/Minimap.h` + `src/ui/Minimap.cpp` — Add a new public method
...
- [x] **MM-15** `src/ui/Minimap.cpp` — constructor: Replace the single `m_toggleBtn` button
...
- [x] **MM-16** `src/ui/Minimap.cpp` — constructor: Create `m_labelStrip` as a static-text
  element at virtual x:1720, y:832, w:200, h:16 with text colour `#EBF4F6`; start hidden.
  (ref: `architecture/ui-ux/minimap.md` — label strip)

- [x] **MM-17** `src/ui/Minimap.cpp` — constructor: Resize the legend panel `m_legendPanel`
...
- [x] **MM-18** `src/ui/Minimap.cpp` — constructor: Update `m_mapBg` background colour from
...
- [x] **MM-19** `src/ui/Minimap.cpp` — `drawOverlay()`: Zone color coding via `fillColoredRect`.
...
- [x] **MM-20** `src/ui/Minimap.cpp` — `drawOverlay()`: Road network rendering via
...
- [x] **MM-19b** `src/ui/Minimap.cpp` — `drawOverlay()`: Service Coverage overlay tile tints.
...
- [x] **MM-19c** `src/ui/Minimap.cpp` — `drawOverlay()`: Traffic Congestion overlay speed-band
...
- [x] **MM-21** `src/ui/Minimap.cpp` — `drawOverlay()`: Camera viewport rectangle projection.
...
- [x] **MM-22** `src/ui/Minimap.cpp` — `draw()`: Budget-tick cadence. Cache **all three**
...
- [x] **MM-23** `src/ui/Minimap.cpp` — Legend content — dynamic switching.
...
- [x] **MM-25** `src/ui/Minimap.cpp` — `onEvent()`: Click-to-pan. Map click pixel coordinates
...
- [x] **MM-26** `src/ui/Minimap.cpp` — `onEvent()`: Two-button radio toggle. Hit-test both
...
- [x] **MM-27** `src/ui/Minimap.cpp` — `setCameraState()` implementation: Store the supplied
  `CameraState` in `m_cameraState` for use during the next `draw()` call.

- [x] **MM-28** `src/ui/Minimap.cpp` — `getWidgetFootprint()` implementation: Return the
...
- [x] **MM-29a** `src/ui/ui_constants.h` — Verify that the three minimap widget
...
- [x] **MM-29** `src/ui/UIManager.cpp` — input arbitration: In `UIManager`'s draw/update loop,
...
- [x] **MM-30** `src/ui/UIManager.cpp` — `draw()`: Call `m_minimap->setCameraState(camState)`
...
- [x] **MM-30c** `src/ui/UIManager.cpp` — `update()`: Forward budget ticks to the minimap.
...
- [x] **MM-30a** `src/ui/UIManager.h` + `src/ui/UIManager.cpp`: Declare and implement the new
...
- [x] **MM-30b** `src/rendering/IrrlichtRenderer.cpp` — After `guiEnv->drawAll()` in
...
- [x] **MM-31** `src/ui/UIManager.cpp` — initialization: Call `m_minimap->setPanCallback(...)`,
...
- [x] **MM-32** Root `CMakeLists.txt`: **Verify** that `tests/ui/minimap_overlay_test.cpp` is
...
- [x] **MM-33** `tests/ui/minimap_overlay_test.cpp` — Group 1: Zone color coding (9 tests).
...
- [x] **MM-34** `tests/ui/minimap_overlay_test.cpp` — Group 2: Road network rendering (4 tests).
...
- [x] **MM-35** `tests/ui/minimap_overlay_test.cpp` — Group 3: Camera viewport rectangle
...
- [x] **MM-36** `tests/ui/minimap_overlay_test.cpp` — Group 4: Click-to-pan (6 tests).
...
- [x] **MM-37** `tests/ui/minimap_overlay_test.cpp` — Group 5: Overlay toggle radio behavior
...
- [x] **MM-38** `tests/ui/minimap_overlay_test.cpp` — Group 6: Label strip (7 tests). Verify
...
- [x] **MM-39** `tests/ui/minimap_overlay_test.cpp` — Group 7: Legend panel (13 tests).
...
- [x] **MM-40** `tests/ui/minimap_overlay_test.cpp` — Group 8: Budget-tick cadence (4 tests).
...
- [x] **MM-41** `tests/ui/minimap_overlay_test.cpp` — Group 9: Input footprint (8 tests).
...
- [x] **MM-42** `tests/ui/minimap_overlay_test.cpp` — Group 10: Element leak regression
...
- [x] **MM-43** `tests/ui/minimap_overlay_test.cpp` — Group 11: Overlay tile rendering
...
- [x] `make build` completes without errors or warnings on both Linux and Windows.
- [x] `CMakeLists.txt` extended via `target_sources(ui_tests PRIVATE tests/ui/minimap_overlay_test.cpp)`
  (MM-32); CTest discovers all 76 new cases under the `unit` label.
- [x] All 76 new test cases in `tests/ui/minimap_overlay_test.cpp` (Groups 1–11) pass under
  `make test`.
- [x] All pre-existing unit, integration, and OpenGL tests continue to pass unchanged —
  zero regressions introduced.
- [x] The element-leak regression tests (Group 10, MM-42) run in CI using `NiceMock<MockUIBackend>`
  ...
  called inside `draw()`.
- [x] `IUIBackend::fillColoredRect` (MM-01) is verified present in `MockUIBackend` with a
  `MOCK_METHOD` stub and in `IrrlichtUIBackend` with a real `draw2DRectangle` implementation.
- [x] `CameraState` has `targetX`, `targetZ`, `zoomDistance` fields (MM-05) and
  `CameraController::getCameraState()` populates them (MM-06).
- [x] `src/ui/ui_constants.h` contains `kMinimapWidgetTop = 848`,
  `kMinimapWidgetTopOverlayActive = 732`, and `kMinimapWidgetLeft = 1576` (added in
  Phase 11d Deliverable 3c; MM-29a verifies presence).
- [x] `Minimap::getWidgetFootprint()` returns `UIRect{1576,848,344,232}` when no overlay
...
- [x] Legend panel geometry is 200×100 at virtual x:1720, y:732 (not 80×100 at the former
  position) — verified by Group 7 tests (MM-39).
- [x] Budget-tick cadence is enforced: `queryTile()` is called at most once per budget tick,
