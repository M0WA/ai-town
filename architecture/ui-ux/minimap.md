...
- **Overlay toggle**: An extensible icon-button row on the minimap border; **one button per overlay type** (radio behavior — exactly one overlay active at a time; clicking a different button switches the overlay). For V1 with Map, Traffic, and Service overlays, the row has three toggle buttons (Map, Traffic, Service) with mutual-exclusivity (radio behavior — exactly one active at a time). This architecture scales to additional overlays (demand heat map, etc.) without UX redesign.
