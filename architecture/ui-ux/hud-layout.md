...
maps to approximately 10–11 physical pixels at 1280×720 (scale factor ≈ 0.667), satisfying the
floor.
...
**Phase assignment**: Font loading is a **Phase 10** deliverable (first phase that delivers
playable gameplay polish). It is not required for Phase 9b (world interaction) correctness, but
its absence causes the unreadable font symptom players experience in Phase 9b builds. Phase 10
MUST include `assets/fonts/hud_font.xml` in the asset manifest and add the constructor call above
to `IrrlichtUIBackend`.
...
1. **Font size** (Phase 10): load `assets/fonts/hud_font.xml` in `IrrlichtUIBackend` as described
   in the Font Loading section above. This makes the fallback text readable even before the sprite
   sheet is present.
