...
## Approved mock policy deviation — HUDGlacierGlassTest

`NiceMock<MockUIBackend>` is used (not `StrictMock`) because the `HUD` constructor emits dozens of `addStaticText`, `addButton`, and `setElementRect` calls during initialization that `StrictMock` would reject without exhaustive setup.

`Times()` qualifiers on critical `EXPECT_CALL` assertions compensate for this relaxation: each meaningful interaction is explicitly bounded.

## Approved mock policy deviation — NotificationRailTest
