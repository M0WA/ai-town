...
- [x] **A-1** _(REN-4 + REN-10)_ The `m_driver->setMaterial(SMaterial{})` flush call is
...
- [x] **A-2** _(REN-1 + REN-3)_ Extract a private helper template
...
- [x] **A-3** _(REN-5)_ Promote the `extractBasename` lambda in `TextureCache::loadSRGB()`
...
- [x] **A-4** _(REN-2)_ Extract `flattenFootprint(tileX, tileZ, footprintN, outTargetH)` and
...
- [x] **A-5** _(REN-6)_ Extract `appendTerrainQuad(buf, tx, tz, colour, yOffset)` and
...
- [x] **A-6** _(REN-9)_ Add a file-scope `static irr::video::SColor argbToSColor(uint32_t argb)`
  helper in `IrrlichtRenderer.cpp` and replace the four separate ARGB bit-shift/mask blocks
  with calls to it. _(REN-9)_

- [x] **A-21** _(REN-26)_ Remove `using namespace irr;` from `SceneEntityManager.h` (file scope
...
- [x] **A-22** _(REN-27)_ Extract `void logWarning(const std::string& msg)` and
...
- [x] **A-33** _(REN-36)_ Split `IrrlichtRenderer::placeRoad()` (~170 lines) into three
...
- [x] **A-7** _(REN-12)_ Replace the nested ternary `(footprintSize < 1) ? 1 : (footprintSize > 3) ? 3 : footprintSize`
  with `std::clamp(footprintSize, 1, 3)` (C++17). _(REN-12)_

- [x] **A-8** _(REN-13)_ Replace the two `while`-loop heading normalisations in
  `moveVehicleAgent()` with `std::fmod(headingDeg, 360.0f); if (h < 0.0f) h += 360.0f;`. _(REN-13)_

- [x] **A-9** _(REN-14)_ Replace the per-tile `std::sqrt` call in `showServiceCoverageOverlay()`
  with a squared-distance comparison; hoist `const float radiusSq = radiusM * radiusM`
  before the loop. _(REN-14)_

- [x] **A-10** _(REN-11)_ Remove the redundant post-loop `node->getMaterial(0).setTexture(0, vehicleTex)`
  call in `spawnVehicleAgent()` that contradicts the conditional loop immediately above it. _(REN-11)_

- [x] **A-11** _(REN-8)_ Change the signal node key in `setIntersectionSignalState()` from
  `int key = tileX * 10000 + tileZ` to `uint64_t key = tileKey(tileX, tileZ)`, matching
  all other tile-keyed registries. _(REN-8)_

- [x] **A-12** _(REN-17)_ Promote the cloud dome geometry constants (`kDomeRings`,
...
- [x] **A-13** _(REN-18)_ Define `static constexpr float kDegToRad = static_cast<float>(M_PI / 180.0)`
  near the top of `IrrlichtRenderer.cpp` and replace the inline cast expression in `setFOV`. _(REN-18)_

- [x] **A-14** _(REN-7)_ Move the road LOD distance constants (`kRoadLOD0to1`,
...
- [x] **A-15** _(REN-15)_ Either use the per-mip `estimatedVRAM` accumulator as the stored
...
- [x] **A-16** _(REN-16)_ Guard test-only accessors in `IrrlichtRenderer.h`
...
- [x] **A-17** _(REN-24)_ Name the overlay Y-offset magic literals in
...
- [x] **A-18** _(REN-20)_ Hoist the repeated `std::string(AITOWN_ASSETS_DIR) + "/3d/vehicles/"` prefix
  out of the `vehicleMeshPath()` switch arms into a single `const std::string prefix` variable. _(REN-20)_

- [x] **A-23** _(REN-28)_ Replace all `unordered_map::count(k) > 0` / `unordered_set::count(k) > 0`
...
- [x] **A-24** _(REN-30)_ Change the three `std::vector<std::string>&` parameters in
...
- [x] **A-25** _(REN-32)_ Extract a `static std::string vehicleAtlasPath(int variantIndex)`
...
- [x] **A-26** _(REN-35)_ Add `[[nodiscard]]` to `ITerrainQuery::getHeightAt()` and
  `getSlopeDegrees()` virtual declarations (pure queries with no side-effects). _(REN-35)_

- [x] **A-27** _(TER-1)_ Replace the `std::stable_sort` on the full rebuild queue after every
...
- [x] **A-28** _(TER-2)_ Replace the 2-branch if-else chains in `TerrainSystem::lodSwitchOutDistance()`
...
- [x] **A-34** _(REN-37)_ Replace the 4-branch if/else-if heading-degree binning in
...
- [x] **A-35** _(TER-3)_ Replace the 3-branch if/else-if LOD→grid-size mapping in
...
- [x] **A-19** _(REN-19)_ Define a `struct PoolEntryBase { int ref_count; uint64_t lastAccessTimestamp; size_t vramBytes; }`
...
- [x] **A-20** _(REN-21, REN-22, REN-23, REN-25)_ Minor clean-up: add a comment explaining
...
- [x] **A-29** _(REN-29)_ Promote `kBlockedArgb` from a function-local `constexpr` to
  `render_constants.h` alongside the other zone-colour ARGB constants. _(REN-29)_

- [x] **A-30** _(REN-31)_ Add `[[nodiscard]]` to `LODNode::getCurrentLOD()` and
  `getMesh()` (pure queries, no side-effects). _(REN-31)_

- [x] **A-31** _(REN-33)_ Add `[[nodiscard]]` to `RenderSystem::maxTextureSize()`,
  `supportsS3TC()`, and `supportsAnisoFiltering()` (device-queried values, no side-effects). _(REN-33)_

- [x] **A-32** _(REN-34)_ Either make `IRenderer::setZoneHoverColour()` pure-virtual (`= 0`)
...
- [x] **A-36** _(REN-38)_ Verify and update the `"Lighting=false: no light nodes in scene
...
- [x] **A-37** _(REN-39)_ Apply the same stale-comment check to the equivalent
  `"no light nodes in the scene yet (Phase 6+)"` comment in `placeBuildingMesh()`. _(REN-39)_

- [x] **A-38** _(REN-40)_ Remove or update the Phase 5/6 terrain-texture commentary in
...
- [x] **B-1** _(AUD-1, AUD-2, AUD-3)_ Add `alCheckError()` after every AL call that
...
- [x] **B-2** _(AUD-4)_ Extract a private `int acquireSFXSlot(SoundPriority priority)`
...
- [x] **B-4** _(AUD-12)_ Delete `src/audio/sound_ids.h` (stale predecessor). Update
...
- [x] **B-5** _(AUD-5)_ Define `constexpr float kDuckRampUpSeconds = 0.2f` and
...
- [x] **B-6** _(AUD-7)_ Define `constexpr float kVehicleEnginePitchIdle = 0.75f` and
...
- [x] **B-7** _(AUD-6)_ Fix the `path.substr(path.size() - 4)` pattern in
...
- [x] **B-25** _(AUD-28)_ Remove the stale `"Phase 3 stubs — no-op. Phase 7 replaces with real
...
- [x] **B-26** _(AUD-29)_ Remove the three-point stale responsibility list inside
...
- [x] **B-27** _(AUD-30)_ Remove the `"NOTE: Phase 10 deliverable … is also a Phase 10
...
- [x] **B-8** _(AUD-8)_ Apply `[[nodiscard]]` to all acquire/find/load/decode helpers in
  `AudioSourcePool.h`, `AudioSystem.h`, and `AudioStream.h`. _(AUD-8)_

- [x] **B-9** _(AUD-10)_ Replace the `goto cleanup_alc` in the `AudioSystem` destructor with
  an `if (m_contextMadeCurrent)` block, removing the `goto` and the label. _(AUD-10)_

- [x] **B-10** _(AUD-14, AUD-15)_ Extract `startCrossfadeIncomingStream(slot, path, isStem, errorOut)`
...
- [x] **B-11** _(AUD-13)_ Merge the first two `m_streamMutex` lock scopes in the main-menu
...
- [x] **B-12** _(AUD-16)_ Cache the `getTotalFrames()` result in a local variable in
  `processPreloadCommand()` and reuse it; add `static_assert` that
  `SFX_ZONE_RESIDENTIAL < SFX_ZONE_INDUSTRIAL`. _(AUD-16)_

- [x] **B-13** _(AUD-20)_ Forward-declare `class AudioSystem` in `AudioSourcePool.h` and
  change `m_audioSystem` / constructor parameter from `void*` to `AudioSystem*`. _(AUD-20)_

- [x] **B-14** _(AUD-21)_ Load `m_sfxVolume` once into a local variable before the two gain
  calls in the per-frame vehicle engine update block. _(AUD-21)_

- [x] **B-15** _(AUD-9)_ Mark `computeSamplesPlayed` `noexcept` and `[[nodiscard]]` in
  `AudioSystem.h`. _(AUD-9)_

- [x] **B-16** _(AUD-17)_ Add a comment to `loadMusicSidecar()` documenting the parse
...
- [x] **B-17** _(AUD-18)_ Update the `m_stingerLastTriggerTime` comment with a concrete
  index example. Add `static_assert(kStingerCount == 2)` adjacent to the array. _(AUD-18)_

- [x] **B-18** _(AUD-19)_ Rename `AudioSourcePool::SFXSourceSlot` to `PoolSFXEntry` (or
  similar) to distinguish it from `AudioSystem::SFXSlot`. _(AUD-19)_

- [x] **B-28** _(AUD-31)_ Update the comment block above `alCheckError_real` / `alcCheckError_real`
...
- [x] **B-29** _(AUD-32)_ Replace `std::max(0.0f, std::min(1.0f, t))` in
  `applyCrossfadeGains()` with `std::clamp(t, 0.0f, 1.0f)` (C++17, already used elsewhere). _(AUD-32)_

- [x] **B-30** _(AUD-33)_ Extract a private `void setupNonPositionalSource(int sourceIdx)`
...
- [x] **B-31** _(AUD-34)_ Define `constexpr` `kAmbientCrossfadeDurationSeconds` separately
...
- [x] **B-32** _(AUD-35)_ Reuse `findEvictionCandidate()` (or extract a shared private helper)
...
- [x] **B-33** _(AUD-36)_ Move the `AudioStream` struct definition from `AudioSystem.h` to
...
- [x] **B-39** _(AUD-42)_ Extract the bar-boundary tracking block inside `refillStream()`
...
- [x] **B-40** _(AUD-43)_ Extract the duplicate idle-source and move-source AL state
...
- [x] **B-41** _(AUD-44)_ Extract the per-frame pitch/gain/position update block from
...
- [x] **B-19** _(AUD-22)_ Mark all `AudioStreamUtils` functions `[[nodiscard]]` and
  `noexcept`. _(AUD-22)_

- [x] **B-20** _(AUD-23)_ Replace the `sizeof`-based EFX static asserts with
  `std::is_same_v<LPALGENFILTERS_t, LPALGENFILTERS>`. _(AUD-23)_

- [x] **B-21** _(AUD-24)_ Move `m_rng` seeding from the member default initialiser to the
...
- [x] **B-22** _(AUD-25)_ Change logging helper signatures to accept `std::string_view`
  instead of `const std::string&`. _(AUD-25)_

- [x] **B-23** _(AUD-26)_ Wrap `OggVorbis_File*` in a RAII holder in `openStreamOGG()` so
  the pointer is freed on all exit paths including exception paths. _(AUD-26)_

- [x] **B-24** _(AUD-27)_ Add
  `static_assert(std::is_trivially_copyable_v<DuckState>, "DuckState must be trivially copyable")`
  in `AudioSystem.cpp`. _(AUD-27)_

- [x] **B-34** _(AUD-37)_ Rename the `src2` local variable in the main-menu EOF branch of
  `refillStream()` to `mainMenuSrc` to distinguish it from the outer `src` loop variable. _(AUD-37)_

- [x] **B-35** _(AUD-38)_ Remove the stale `"use AudioSourcePool logic inline (pool object is
...
- [x] **B-36** _(AUD-39)_ Define `constexpr std::chrono::milliseconds kAudioThreadWakeInterval{10}`
...
- [x] **B-37** _(AUD-40)_ Replace the manual `alGetError()` pre-clear + post-check pattern in
  `allocateEFXFilters()` with `alCheckError()` calls to match the convention used everywhere
  else in the file. _(AUD-40)_

- [x] **B-38** _(AUD-41)_ Add `static_assert(sizeof(ALuint) == sizeof(unsigned int))` adjacent
...
- [x] **B-42** _(AUD-45)_ Extract the pitch and gain crossblend calculation in
...
- [x] **C-1** _(SIM-1)_ Extract `startingFundsForDifficulty(Difficulty)` and
...
- [x] **C-2** _(SIM-2)_ Extract a private `resetTrafficWindows()` method containing the
  traffic rolling-window zero-initialisation and call it from both the constructor and
  `reset()`. _(SIM-2)_

- [x] **C-3** _(SIM-3)_ Compute `travelTime` once before the three zone-type sample blocks
...
- [x] **C-4** _(SIM-5)_ Move the "which service types exist" boolean scan in
  `doDesirabilityTick()` above the tile loop so it runs once per tick, not once per tile. _(SIM-5)_

- [x] **C-5** _(SIM-6)_ Remove the three redundant `m_taxRates[0/1/2] = 0.05f` assignments
  from the constructor body; the in-class initialiser already sets the correct default. _(SIM-6)_

- [x] **C-6** _(SIM-7)_ Add a `computeEconomySnapshot()` first-pass method that writes
...
- [x] **C-27** _(SIM-27)_ Split `doDensityUnlockTick()` (~295 lines) into focused private
...
- [x] **C-7** _(SIM-4)_ Remove the redundant per-access modulo in the traffic rolling-window
...
- [x] **C-8** _(SIM-8)_ Add population-capacity constants (`max_pop_residential_low`,
  `max_pop_residential_medium`, etc.) to `SimulationConstants` and reference them from
  `maxPopulationForTile()`. _(SIM-8)_

- [x] **C-9** _(SIM-9)_ Add city-rating population threshold constants to `SimulationConstants`
  (`city_rating_town_threshold`, etc.) and use them in `checkCityRatingTransition()`. _(SIM-9)_

- [x] **C-10** _(SIM-10)_ Replace the bare `3.14159265f` literal in the heading calculation
  with `std::numbers::pi_v<float>` (C++20) or a `constexpr float kPiF`. _(SIM-10)_

- [x] **C-11** _(SIM-11)_ Add `static constexpr int kDespawnedVehicleTile = -9999` and
  optionally a `TrafficVehicle::isDespawned()` predicate; replace the three literal
  occurrences. _(SIM-11)_

- [x] **C-12** _(SIM-14)_ Add `constexpr float SimulationConstants::loan_interest_rate = 0.05f`
  and reference it from `processLoanRepayments()` instead of the bare `0.05f` literal. _(SIM-14)_

- [x] **C-13** _(SIM-13)_ Replace the index-collection + reverse-loop erase in
  `processLoanRepayments()` with `std::erase_if` (C++20) or the erase-remove idiom. _(SIM-13)_

- [x] **C-14** _(SIM-15)_ Cache the BFS result of `computePowerCoverage()` per tick: add
...
- [x] **C-15** _(SIM-16)_ Replace the private `ServiceType` enum with the public
  `ServiceBuildingType` enum throughout `CitySimulation`'s private data. _(SIM-16)_

- [x] **C-16** _(SIM-17)_ Change `SimulationConstants::startingFundsForDifficulty()` to
...
- [x] **C-17** _(SIM-12)_ Move `setMapDimensions()` body from the header to
  `CitySimulation.cpp`. _(SIM-12)_

- [x] **C-24** _(SIM-24)_ Replace the three fixed-length C-arrays `m_taxRates[3]`,
...
- [x] **C-25** _(SIM-25)_ After C-24, replace the three explicit per-index zero-assignments in
  `reset()` (`m_lastMonthTaxRevenue[0/1/2] = 0.0f`) with a single `.fill(0.0f)` call. _(SIM-25)_

- [x] **C-26** _(SIM-26)_ Replace the 5-branch if-else time-of-day classifier in `tick()` with a
...
- [x] **C-28** _(SIM-28)_ Replace the 6-case `switch` mapping `tierIdx` to `(zone, density)`
...
- [x] **C-29** _(SIM-29)_ Split `doDesirabilityTick()` (~158 lines) by extracting the
...
- [x] **C-30** _(SIM-30)_ Split `doPopulationTick()` (~72 lines) by extracting
...
- [x] **C-18** _(SIM-18)_ Replace the C-string pointer switches in `zoneAssetBaseName()`
  with a 2D `constexpr const char*` lookup table. _(SIM-18)_

- [x] **C-19** _(SIM-19)_ Move file-scope `kVehicleTilePerSecond` and `kVehicleSpawnInterval`
  to `SimulationConstants`; remove the duplicate `kTileSizeM` and use `kTileSizeMeters`
  consistently. _(SIM-19)_

- [x] **C-20** _(SIM-20)_ Evaluate the `AITOWN_TESTING_ENABLED` guard pattern in
  `CitySimulation.h` and apply a CMake-level enforcement to prevent the macro from being
  set in non-test builds. _(SIM-20)_

- [x] **C-21** _(SIM-21)_ Remove the redundant `#if defined(_WIN32) / #else / #endif`
  block that includes the same `<cstdlib>` header in both branches in `SaveSystem.cpp`. _(SIM-21)_

- [x] **C-22** _(SIM-22)_ Replace the platform-discriminated path-string construction in
  `slotFilePath()` and `autoSaveFilePath()` with `std::filesystem::path(dir) / filename`. _(SIM-22)_

- [x] **C-23** _(SIM-23)_ Change the `///` Doxygen-style comment in `ITerrainQuery.h`
...
- [x] **D-1** _(UI-1)_ Create `src/ui/ui_format.h` + `src/ui/ui_format.cpp` with a single
...
- [x] **D-2** _(UI-4)_ Remove `const` from `KeyBindings::undo` and `KeyBindings::save` to
...
- [x] **D-3** _(UI-5)_ Move `KeyBindings::writeToFile()` body from the header to a new
...
- [x] **D-4** _(UI-3)_ Extract `WorldInteractionState` (activeTool, hoveredTile, zoneAnchor,
...
- [x] **D-5** _(UI-2)_ Promote `ratingName(CityRatingTier)` from a file-local static in
  `HUD.cpp` to a free function declared in `src/ui/ui_format.h` (or `simulation_types.h`). _(UI-2)_

- [x] **D-23** _(UI-23)_ Split `UIManager::update()` (~397 lines) into private sub-methods:
...
- [x] **D-6** _(UI-6)_ Remove the redundant `static` keyword from
...
- [x] **D-7** _(UI-8)_ Remove `getLastResult()` from `ModalDialog`'s public interface;
  rename or add `peekResult() const noexcept` if test-only read access is needed. _(UI-8)_

- [x] **D-8** _(UI-9)_ Mark `UIManager::setOverlayMapForTest()` with
  `[[deprecated("for tests only")]]`. Group all test seams under a single `// TEST SEAM API`
  section with a contract comment. _(UI-9)_

- [x] **D-9** _(UI-10)_ Move the 12 layout constants from `NotificationManager.h`'s class
  body to `NotificationManager.cpp` as anonymous-namespace `constexpr` values. _(UI-10)_

- [x] **D-10** _(UI-11)_ Mark `UIScaler` members that never change after construction
  (`m_virtualW`, `m_virtualH`, `m_offsetX`, `m_offsetY`) as `const int`. _(UI-11)_

- [x] **D-11** _(UI-13)_ Replace `FinancesPanel`'s three named `ZoneRow` members
  (`m_rowR`, `m_rowC`, `m_rowI`) with `std::array<ZoneRow, 3> m_rows` indexed by
  `(int)ZoneType`. _(UI-13)_

- [x] **D-12** _(UI-14)_ Replace `UIManager`'s `UIElementHandle m_zoneSubPanelBtns[9]`
  and `m_utilSubPanelBtns[4]` C-arrays with `std::array<UIElementHandle, 9>` and
  `std::array<UIElementHandle, 4>` respectively. _(UI-14)_

- [x] **D-13** _(UI-7)_ Move the `PendingQuitAction` enum class from `UIManager`'s private
  member section to `ui_types.h`. _(UI-7)_

- [x] **D-14** _(UI-12)_ Move `CameraController::resetOrbit()` body from the header to
  `CameraController.cpp`. _(UI-12)_

- [x] **D-15** _(UI-18)_ Add `constexpr float kDefaultMasterVolume = 1.0f`,
...
- [x] **D-16** _(UI-19)_ Rename `ICitySimulation::getDemandPressurePct()` →
...
- [x] **D-21** _(UI-21)_ Declare `m_bindings` as `const KeyBindings` in `CameraController.h`;
  it is set once in the constructor initialiser list and never reassigned. _(UI-21)_

- [x] **D-22** _(UI-22)_ Change the `m_overlayMap` key computation in `UIManager` from `int`
...
- [x] **D-17** _(UI-15)_ Add a null check or `assert` to `Minimap::setSimulation()` and
  a mode-guard to `setOverlayMode()` (fallback to `None` if traffic mode is requested
  while `m_sim == nullptr`). _(UI-15)_

- [x] **D-18** _(UI-16)_ Apply `[[nodiscard]]` to `consumeSaveRequest()`,
  `consumeQuitRequest()`, and similar `pollFlag`-style methods in `PauseMenuPanel` and
  `MainMenuPanel`. _(UI-16)_

- [x] **D-19** _(UI-17)_ Rename `Rect` in `IUIBackend.h` to `UIRect` to avoid collision with
  `irr::core::rect` and platform-level `Rect` types. _(UI-17)_

- [x] **D-20** _(UI-20)_ Replace the double-duty `amount`/`repaymentTicks` fields in
...
- [x] **E-1** _(BUILD-3)_ Remove the duplicate `target_sources()` block that adds
...
- [x] **E-2** _(BUILD-4)_ Change `target_link_libraries(aitown_terrain PUBLIC aitown_render)`
  to `PRIVATE` to stop leaking Irrlicht and GLEW headers into the terrain test graph. _(BUILD-4)_

- [x] **E-3** _(BUILD-2)_ Audit the `AITOWN_TESTING_ENABLED=1` compile definition on
...
- [x] **E-4** _(BUILD-1)_ Create a CMake `INTERFACE` library `aitown_coverage_flags` and
...
- [x] **E-5** _(BUILD-5)_ Extract a CMake macro `aitown_copy_runtime_dlls(target)` for
  the Windows DLL post-build copy pattern. Replace the four verbatim `add_custom_command`
  blocks. _(BUILD-5)_

- [x] **E-6** _(BUILD-6)_ Create a CMake `INTERFACE` library `aitown_irrlicht_system_deps`
...
- [x] **E-7** _(BUILD-7)_ Document the `package-windows` intent explicitly: either add a
...
- [x] **E-16** _(BUILD-16)_ Add `/opt/vcpkg_installed/*` (and `$VCPKG_ROOT/installed/*`)
  to the Makefile `lcov --remove` exclusion list to match the CI pattern set. _(BUILD-16)_

- [x] **E-17** _(BUILD-17)_ Replace the twelve individual "Verify check_N present" steps in
  `_validate-assets.yml` with a single loop step. _(BUILD-17)_

- [x] **E-18** _(BUILD-18)_ Evaluate replacing the `prepare` serial job in `ci.yml` with
  GitHub Actions `vars` context or direct string literals to remove the serial critical-path
  job. _(BUILD-18)_

- [x] **E-19** _(BUILD-19)_ Replace `${{ github.workspace }}` in the lcov `--remove` pattern
  in `_coverage-linux.yml` with a shell-resolved `$(pwd)` variable. _(BUILD-19)_

- [x] **E-20** _(BUILD-20)_ Add `"VCPKG_TARGET_TRIPLET": "x64-windows"` explicitly to the
  `ci-windows` preset in `CMakePresets.json`. _(BUILD-20)_

- [x] **E-21** _(BUILD-21)_ Add `name:` labels to the `ilammy/msvc-dev-cmd` and
  `lukka/run-vcpkg` steps in `_package-windows.yml`. _(BUILD-21)_

- [x] **E-30** _(BUILD-30)_ Remove all `"Phase N:"` / `"Phase N delivers..."` / `"Phase N
...
- [x] **E-31** _(BUILD-31)_ Renumber (or remove) the out-of-sequence `# Step N:` step annotations
...
- [x] **E-32** _(BUILD-32)_ Remove stale prose from `_coverage-linux.yml`: the `"lcov --summary
...
- [x] **E-35** _(BUILD-35)_ Hoist the repeated `mkdir -p test_results` directory-creation
...
- [x] **E-22** _(BUILD-22)_ Add an explicit configure-time error if `XXFV86VM_LIB` is not
...
- [x] **E-23** _(BUILD-23)_ Remove the redundant `if: runner.os == 'Linux'` condition from
  the `ccache` step in `_build-linux.yml` and `_coverage-linux.yml`. _(BUILD-23)_

- [x] **E-24** _(BUILD-24)_ Deduplicate the `empty` token in the Makefile `lcov --capture`
  `--ignore-errors` list; synchronise the flag set with `_coverage-linux.yml`. _(BUILD-24)_

- [x] **E-25** _(BUILD-25)_ Pin the `docker/ci-linux/Dockerfile` base image from
...
- [x] **E-26** _(BUILD-26)_ Pin `mutagen` and `Pillow` to specific versions in the
  `_validate-assets.yml` pip install step, or extract a `tools/requirements-validate.txt`
  file. _(BUILD-26)_

- [x] **E-27** _(BUILD-27)_ Replace the global `npm install -g markdownlint-cli` in
...
- [x] **E-28** _(BUILD-28)_ Update `actions/download-artifact` pin to the same minor
  version (`v4.6.0`) as `actions/upload-artifact`. _(BUILD-28)_

- [x] **E-29** _(BUILD-29)_ Remove the manual `OpenAL::OpenAL` IMPORTED target workaround
...
- [x] **E-33** _(BUILD-33)_ Collapse the per-phase check-number inventory in the
...
- [x] **E-34** _(BUILD-34)_ Remove the `"(Phase 7)"` parenthetical hardening annotation from
  the DLL verification step comment in `_build-windows.yml`. _(BUILD-34)_

- [x] **E-36** _(BUILD-36)_ Refactor the `all-checks-pass` gate in `ci.yml` to iterate over a
...
- [x] **F-1** _(TEST-1)_ In the `cs()` downcast helpers in `population_test.cpp`,
...
- [x] **F-2** _(TEST-2)_ Extract `NiceSimulationTestBase` (mirroring `SimulationTestBase` but
...
- [x] **F-3** _(TEST-3)_ Extract a `UITestFixtureBase` class (or free helper
...
- [x] **F-4** _(TEST-4)_ Centralise `makeKeyDown`, `makeClick`, `makeMouseButtonDown`, and
...
- [x] **F-5** _(TEST-5)_ In `UIManagerModalTest::SetUp()` (line ~41), replace the `nullptr`
...
- [x] **F-6** _(TEST-6)_ Rename all `*_InPhase3Stub` and `*_Phase3*` test names in
...
- [x] **F-7** _(TEST-7)_ Either implement real assertions in the `EscapeClosesSettings*` stub
...
- [x] **F-8** _(TEST-8)_ Replace the four `"Phase 10:"` comment prefixes in
...
- [x] **F-9** _(TEST-9)_ Remove `"Phase N stub"` / `"Phase 6 stub — full impl in Phase 11"`
...
- [x] **F-10** _(TEST-10)_ Replace hardcoded fund literals (`500000.0f`, `1000000.0f`,
...
- [x] **F-11** _(TEST-11)_ Add at least one non-trivial `EXPECT_*` assertion to each
...
- [x] **F-12** _(TEST-12)_ Remove the 16 `"Phase 3 stub bodies"` / `"Phase 3 no-ops"` comment
...
- [x] **F-13** _(TEST-13)_ Unify `NiceCoverageTest` and `NiceExtraCoverageTest` in
...
- [x] **F-14** _(TEST-14)_ In `EconomyTest`, either switch `audio_` from `StrictMock` to
...
- [x] **F-15** _(TEST-15)_ Derive `PlacementConflictTest` from `SimulationTestBase` instead
  of reimplementing all 8 `EXPECT_CALL` suppressions and construction boilerplate (~35 duplicate
  lines). _(TEST-15)_

- [x] **F-16** _(TEST-16)_ Rename the `UIManagerModalTest_Phase8` fixture class to
  `UIManagerModalBehaviourTest` (or similar) to remove the `_Phase8` delivery watermark from CI
  test output. _(TEST-16)_

- [x] **F-17** _(TEST-17)_ Remove ordinal delivery labels (`"Method 19 — Phase 10 addition"`,
...
- [x] **F-18** _(TEST-18)_ Update the `save_system_test.cpp` file header which says
...
- [x] All HIGH-priority deliverables (A-1–A-6, A-21–A-22, A-33, B-1–B-7, B-25–B-27,
  C-1–C-6, C-27, D-1–D-5, D-23, E-1–E-7, F-1–F-4) are implemented and all tests pass
  (`make test`).
- [x] All MEDIUM-priority deliverables are implemented or explicitly deferred with a
  documented reason.
- [x] All existing unit, integration, and OpenGL tests pass unchanged under
  `make test` — no regressions introduced by the refactoring.
- [x] `npx markdownlint-cli 'architecture/**/*.md' 'implementation/*.md' 'CLAUDE.md'`
  reports zero errors.
- [x] No new spec contradictions are introduced by the renames (specifically: if
