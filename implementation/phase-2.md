...
- `LODSwapSmokeTest.SetMeshGrabDropContract` is registered under `requires-opengl` label and produces SKIP (not FAIL) under `xvfb-run`
  > RE-VERIFIED 2026-02-23 by prod-owner — `tests/rendering/lod_swap_smoke_test.cpp:19-27` contains `TEST(LODSwapSmokeTest, SetMeshGrabDropContract)` with exact `GTEST_SKIP() << "LOD swap timing requires real GPU; promoted to Phase 5."` body (line 26). Fixed: `CMakeLists.txt:486-490` `add_executable(opengl_tests ...)` now lists `tests/rendering/lod_swap_smoke_test.cpp` as an actual source entry (was a comment-only placeholder — fixed in re-verification pass). `aitown_add_tests(opengl_tests LABEL "requires-opengl")` registers under `requires-opengl` label. PASS.
