---
name: fix-tech-implementation
description: Iteratively fix all CRITICAL/HIGH issues in both specs and the implementation plan until a clean pass is achieved — using only the tech squad (GitHub Pipeline Engineer, Irrlicht dev, OpenAL Soft dev, Test Engineer). Domain-scoped re-reviews, diff-based prompts, issue deduplication, fix verification, and deferred commits. Does NOT sync the plan from specs first.
---

# Fix Tech Implementation

Iteratively review and fix issues across both the spec files and the implementation plan —
repeating until a full clean pass is achieved on the targeted phases. Only the tech squad
...
**Round 1**: run all 4 tech squad agents (full read, no diff available yet).
...
```
=== TECH IMPLEMENTATION REVIEW — Round N ===
Severity filter: [TARGET_SEVERITIES]
Review model:    [TARGET_MODEL]
Agents run: X / 4  (skipped: [list of skipped agents and reason])
...
Re-run **all 4 tech squad agents** simultaneously using the **Round 1 prompt** (full read, no
...
- If **yes** → all 4 agents confirmed clean on a fresh read; proceed to Step 6.
...
git add -A
git commit -m "fix(specs+plan): apply plan+spec fixes from tech squad review (N rounds)"
```
...
```
=== FIX TECH IMPLEMENTATION COMPLETE ===
...
by all 4 tech squad agents with no [TARGET_SEVERITIES] issues remaining.
```
...
- Only the 4 tech squad agents are used (no design squad agents)
- All 4 agents run on round 1; later rounds are domain-scoped (Step 1)
...
- A "clean pass" only counts as done after the Step 5a verification pass (all 4 agents re-run from scratch) also returns `NO ISSUES FOUND` — if the verification pass finds issues, fix cycles continue
