...
### Phase 4 Missing Named Asset Placeholders

> **Note**: All files in this section are functional synthetic placeholder tones generated by
> SoX v14.4.2. They contain real audio signal (approximately −18 to −28 LUFS depending on
> category — see Tone Design column in each table below) and are NOT production-quality
> deliverables. Their purpose is to satisfy filename-presence checks, OGG/WAV header
> validation (`ov_fopen`, `ov_info`, WAV RIFF header), and `validate_assets.py` format
> checks, AND to exercise the audio subsystem with real signal so that streaming, mixing,
> loudness, and spatialization code paths can be smoke-tested meaningfully before DAW-authored
> assets are available. **Every file in this table MUST be replaced with a DAW-authored
> production asset before Phase 10 exit.**

#### Music Stems (OGG Vorbis, stereo, 44100 Hz, 90.0 s)

| File | Format | Channels | Sample Rate | Duration | JSON Sidecar | Tone Design |
|---|---|---|---|---|---|---|
| `music_main_menu_01.ogg` | OGG Vorbis | 2 (stereo) | 44100 Hz | 90.0 s | `music_main_menu_01.json` | Sine drone: G2 (98 Hz) + D3 (147 Hz) + G3 (196 Hz) — open power chord, −18 LUFS approx. |
| `music_main_menu_02.ogg` | OGG Vorbis | 2 (stereo) | 44100 Hz | 90.0 s | `music_main_menu_02.json` | Sine drone: G2 (98 Hz) + G3 (196 Hz) + D4 (294 Hz) — octave + fifth, −18 LUFS approx. |
| `music_calm_01.ogg` | OGG Vorbis | 2 (stereo) | 44100 Hz | 90.0 s | `music_calm_01.json` | Sine drone: A2 (110 Hz) + E3 (165 Hz) + A3 (220 Hz) — calm open fifth/octave, −18 LUFS approx. |
| `music_calm_02.ogg` | OGG Vorbis | 2 (stereo) | 44100 Hz | 90.0 s | `music_calm_02.json` | Sine drone: A2 (110 Hz) + A3 (220 Hz) + E4 (330 Hz) — octave + major sixth, −18 LUFS approx. |
| `music_growth_01.ogg` | OGG Vorbis | 2 (stereo) | 44100 Hz | 90.0 s | `music_growth_01.json` | Sine drone: C3 (131 Hz) + G3 (196 Hz) + E4 (330 Hz) — bright major triad, −18 LUFS approx. |
| `music_growth_02.ogg` | OGG Vorbis | 2 (stereo) | 44100 Hz | 90.0 s | `music_growth_02.json` | Sine drone: C3 (131 Hz) + C4 (262 Hz) + E4 (330 Hz) — major octave+third, −18 LUFS approx. |
| `music_crisis_01.ogg` | OGG Vorbis | 2 (stereo) | 44100 Hz | 90.0 s | `music_crisis_01.json` | Sine drone: D3 (147 Hz) + Ab3 (208 Hz) — tritone tension, −18 LUFS approx. |
| `music_crisis_02.ogg` | OGG Vorbis | 2 (stereo) | 44100 Hz | 90.0 s | `music_crisis_02.json` | Sine drone: D2 (73 Hz) + D3 (147 Hz) + Ab3 (208 Hz) — deep tritone with sub, −18 LUFS approx. |

Each JSON sidecar contains `{"bpm":90,"beats_per_bar":4}` and satisfies `validate_assets.py`
Check #14 (music sidecar presence) and `music_sidecar_schema.json` schema validation.
The 90.0 s duration is within the 90–180 s spec range and is a round bar-boundary multiple
(at 90 BPM, 4/4: 1 bar = 2.667 s; 33.75 bars × 2.667 s ≈ 90.0 s — acceptable for placeholder
purposes; production stems must be authored to an exact integer bar count).
All stems carry a 2 s fade-in and 2 s fade-out to exercise the streaming and mixing
code paths with a non-silent envelope.

#### Ambient Beds (OGG Vorbis, stereo, 44100 Hz, 90.0 s, no JSON sidecar)

| File | Format | Channels | Sample Rate | Duration | JSON Sidecar | Tone Design |
|---|---|---|---|---|---|---|
| `ambient_day.ogg` | OGG Vorbis | 2 (stereo) | 44100 Hz | 90.0 s | None (exempt) | Pink noise, −20 LUFS approx. — suggests city hum and outdoor ambience. |
| `ambient_night.ogg` | OGG Vorbis | 2 (stereo) | 44100 Hz | 90.0 s | None (exempt) | Pink noise, −23 LUFS approx. — quieter than day to suggest reduced nighttime activity. |
| `ambient_dawn.ogg` | OGG Vorbis | 2 (stereo) | 44100 Hz | 90.0 s | None (exempt) | Pink noise, −21 LUFS approx. — between night and day levels. |
| `ambient_dusk.ogg` | OGG Vorbis | 2 (stereo) | 44100 Hz | 90.0 s | None (exempt) | Pink noise, −22 LUFS approx. — between day and night levels. |

All four ambient beds use a 3 s fade-in and 3 s fade-out to exercise the crossfade
streaming code path with a non-silent level envelope. Pink noise is spectrally similar
to real city ambience and will reveal any frequency-dependent artifacts in the
streaming/mixing pipeline during smoke tests.
Ambient beds are explicitly exempt from the JSON sidecar requirement per
`architecture/audio-architecture/v1-audio-asset-manifest.md`.
The 200 ms pre-baked DAW crossfade tail required by the production spec is NOT present
in these placeholders — production assets must carry this pre-baked tail before Phase 10 exit.

#### Zone Loops (OGG Vorbis, mono, 44100 Hz, 15.0 s, no JSON sidecar)

| File | Format | Channels | Sample Rate | Duration | JSON Sidecar | Tone Design |
|---|---|---|---|---|---|---|
| `sfx_zone_residential.ogg` | OGG Vorbis | 1 (mono) | 44100 Hz | 15.0 s | None | Pink noise, −26 LUFS approx. — soft suburban background. 0.5 s fade at head/tail. |
| `sfx_zone_commercial.ogg` | OGG Vorbis | 1 (mono) | 44100 Hz | 15.0 s | None | Pink noise + sine 320 Hz activity hum, −26 LUFS approx. — slight commercial bustle. 0.5 s fade at head/tail. |
| `sfx_zone_industrial.ogg` | OGG Vorbis | 1 (mono) | 44100 Hz | 15.0 s | None | Brown noise, −26 LUFS approx. — heavy machinery rumble character. 0.5 s fade at head/tail. |

Duration 15.0 s satisfies `validate_assets.py` Check #18 constraint (12–18 s hard cap, mono,
44100 Hz). The 0.5 s fade-in and fade-out on each loop exercise the spatialization and
loop-point handling code. The production silence-boundary fade (−60 dBFS head/tail)
will be authored in the DAW for final production assets before Phase 10 exit.

#### WAV SFX One-Shots (PCM 16-bit, mono, 44100 Hz)

| File | Format | Channels | Sample Rate | Duration | JSON Sidecar | Tone Design |
|---|---|---|---|---|---|---|
| `sfx_build_place.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 0.5 s | None | Sine sweep 800→400 Hz — placement thud. −24 LUFS approx. |
| `sfx_build_demolish.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 0.5 s | None | Brown noise burst with 0.3 s fade-out — demolition impact. −24 LUFS approx. |
| `sfx_road_build.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 0.5 s | None | Sine 600 Hz click with 0.3 s fade-out. −24 LUFS approx. |
| `sfx_earthworks.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 0.5 s | None | Brown noise punch with 0.4 s fade-out. −24 LUFS approx. |
| `sfx_zone_upgrade.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 1.0 s | None | Ascending sine sweep 440→880 Hz — positive reward. −22 LUFS approx. |
| `sfx_service_degrade.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 1.0 s | None | Descending sine sweep 880→220 Hz — warning descent. −22 LUFS approx. |
| `sfx_budget_warn.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 1.0 s | None | Sine sweep 440→370 Hz — two-tone budget warning. −24 LUFS approx. |
| `sfx_loan_issued.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 0.5 s | None | Sine 523 Hz (C5) — neutral notification chime. −24 LUFS approx. |
| `sfx_power_out.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 1.0 s | None | Descending sine 200→50 Hz — power-down tone. −22 LUFS approx. |
| `sfx_water_out.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 1.0 s | None | Sine 300 Hz with 0.3 s fade-out. −22 LUFS approx. |
| `sfx_fire_alert.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 2.0 s | None | Sine sweep 880→660 Hz — fire siren character. −18 LUFS approx. |
| `sfx_police_alert.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 2.0 s | None | Sine sweep 960→720 Hz — distinct from fire alert. −18 LUFS approx. |
| `sfx_intersection_tick.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 0.3 s | None | Sine 1200 Hz short click. −28 LUFS approx. — very subtle. |
| `ui_click.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 0.1 s | None | Sine 1000 Hz micro-click. −24 LUFS approx. |
| `ui_toast.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 0.2 s | None | Sine 880 Hz chime. −22 LUFS approx. |
| `ui_menu_open.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 0.2 s | None | Ascending sine sweep 440→660 Hz. −24 LUFS approx. |
| `ui_menu_close.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 0.2 s | None | Descending sine sweep 660→440 Hz. −24 LUFS approx. |
| `sfx_vehicle_horn.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 0.5 s | None | Sine 440 Hz (A4 horn tone). −18 LUFS approx. |

#### Stingers (WAV PCM 16-bit, mono, 44100 Hz)

| File | Format | Channels | Sample Rate | Duration | JSON Sidecar | Tone Design |
|---|---|---|---|---|---|---|
| `stinger_crisis.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 2.0 s | None | Sine dual-tone D3 (147 Hz) + Ab3 (208 Hz) tritone — dissonant tension chord. −18 LUFS approx. 0.1 s fade-in, 0.3 s fade-out. |
| `stinger_milestone.wav` | WAV PCM 16-bit | 1 (mono) | 44100 Hz | 2.0 s | None | Ascending sine sweep 440→880 Hz (A4→A5 octave) — bright achievement sweep. −18 LUFS approx. 0.1 s fade-in, 0.3 s fade-out. |

Mono WAV satisfies `validate_assets.py` Check #19 (stingers must be mono PCM WAV).
`AL_SOURCE_RELATIVE = AL_TRUE` (non-positional) requirement is an engine setting, not an
asset property — these files conform to the mono channel constraint that makes
`AL_SOURCE_RELATIVE = AL_TRUE` safe on all OpenAL Soft implementations.

#### Vehicle Engine Loops (OGG Vorbis, mono, 44100 Hz, 6.0 s)

| File | Format | Channels | Sample Rate | Duration | JSON Sidecar | Tone Design |
|---|---|---|---|---|---|---|
| `sfx_vehicle_engine_idle.ogg` | OGG Vorbis | 1 (mono) | 44100 Hz | 6.0 s | None | Sine harmonics: 55 Hz (fundamental) + 110 Hz + 165 Hz — low-RPM engine rumble character. −22 LUFS approx. |
| `sfx_vehicle_engine_move.ogg` | OGG Vorbis | 1 (mono) | 44100 Hz | 6.0 s | None | Sine harmonics: 80 Hz + 160 Hz + 240 Hz — higher-RPM moving engine tone. −22 LUFS approx. |

Duration 6.0 s meets `kVehicleEngineLoopMinDurationSeconds = 6.0f` exactly (satisfies
Check #17). At the lowest pitch-shift ratio (0.75×) the perceived loop is ~4.5 s —
at the lower bound of the perceptibility threshold. Production assets should target
8–12 s for greater loop naturalness.

#### Sign-Off Checklist

- [x] All 8 music stem OGG files are valid OGG Vorbis, 44100 Hz stereo (2 channels), 90.0 s
- [x] All 8 music stem OGG files contain functional synthetic tones (harmonically related sine drones, −18 LUFS approx.) with 2 s fade-in/fade-out
- [x] All 8 music stem JSON sidecars present with `{"bpm":90,"beats_per_bar":4}`
- [x] All 4 ambient bed OGG files are valid OGG Vorbis, 44100 Hz stereo (2 channels), 90.0 s
- [x] All 4 ambient bed OGG files contain functional synthetic tones (pink noise at −20 to −23 LUFS) with 3 s fade-in/fade-out
- [x] No JSON sidecars created for ambient beds (correctly exempt)
- [x] All 3 zone loop OGG files are valid OGG Vorbis, 44100 Hz mono (1 channel), 15.0 s
- [x] All 3 zone loop OGG files contain functional synthetic tones (pink/brown noise, −26 LUFS) with 0.5 s fade-in/fade-out
- [x] Zone loop durations within 12–18 s hard cap (satisfies Check #18)
- [x] All 18 WAV SFX one-shots are 44100 Hz, mono (1 channel), 16-bit signed PCM
- [x] All 18 WAV SFX one-shots contain functional synthetic tones at appropriate loudness levels (−18 to −28 LUFS depending on category)
- [x] `stinger_crisis.wav` and `stinger_milestone.wav` are 44100 Hz mono PCM 16-bit (satisfies Check #19)
- [x] Both stinger files contain functional synthetic tones (tritone/sweep, −18 LUFS approx.) with 0.1 s attack and 0.3 s release
- [x] Both vehicle engine OGG loops are 44100 Hz mono (1 channel), 6.0 s (meets 6 s minimum — satisfies Check #17)
- [x] Both vehicle engine OGG loops contain functional synthetic tones (sine harmonics simulating engine RPM, −22 LUFS approx.)
- [x] `sfx_vehicle_horn.wav` is 44100 Hz mono PCM 16-bit, 0.5 s (within 0.4–1 s range)
- [x] All soxi spot-checks passed: rate, channels, duration, and encoding verified for all categories
- [x] 4 generic placeholder files upgraded to functional tones (`music_placeholder.ogg`, `ambient_bed_placeholder.ogg`, `placeholder_zone_loop.ogg`, `placeholder_vehicle_engine.ogg`)
- [ ] Replace all placeholder files in this section with production DAW-authored assets before Phase 10 exit
- [ ] Production music stems must be authored to an exact integer bar count at 90 BPM
- [ ] Production ambient beds must carry a 200 ms pre-baked DAW crossfade tail at loop boundary
- [ ] Production zone loops must carry silence-boundary fades (−60 dBFS head and tail)
- [ ] Production vehicle engine loops are recommended at 8–12 s for natural loop quality
