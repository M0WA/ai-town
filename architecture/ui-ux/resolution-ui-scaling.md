...
- `assets/fonts/hud_font.xml` — DejaVu Sans, **22 px physical**
- `assets/fonts/hud_mono_font.xml` — DejaVu Sans Mono, **22 px physical**

Both fonts are generated by `tools/generate_bitmap_fonts.py`. Running
`python3 tools/generate_bitmap_fonts.py` without arguments regenerates all six tier-specific
font pairs (22 px, 33 px, 44 px for both proportional and mono typefaces).
...
icons not text labels. 22 px is the minimum for comfortable HUD readability.
...
**Required layout** (enforced by `tools/generate_bitmap_fonts.py`):

- Cell height = `ascent + descent` from PIL `getmetrics()` (22 px for 22 px DejaVu Sans:
  ascent = 17, descent = 5).
...
## Bitmap Font Tier Selection

Font tier selection occurs **once** at `IrrlichtUIBackend` construction — before any UI element
is created — and is **not dynamic**. Resolution changes during a session are not supported in V1.
...
### kLineH = 33 Virtual Pixel Invariant

The virtual layout constant `kLineH = 33` is valid for **all three resolution tiers**. The math
proof (physical px × reference_height / physical_height = virtual row height):

- `22 × 1080 / 720 = 33.0` (720p tier)
- `33 × 1080 / 1080 = 33.0` (1080p tier)
- `44 × 1080 / 1440 = 33.0` (1440p tier)

No panel layout constants change across tiers — only the physical font glyphs grow to match the
display. See `architecture/ui-ux/query-inspector-panel.md` Panel Layout Constants for usage.

### Selection Implementation

`IrrlichtUIBackend` implements tier selection via a static pure function:
...
The selected tier determines which font XML pair is loaded via
`m_env->getFont("assets/fonts/hud_font_<tier>.xml")`. When the device driver is `EDT_NULL`
(headless CI), font loading is skipped and `m_hudFont` / `m_hudMonoFont` remain `nullptr`.
